...
Para equilibrar expresividad matemática y eficiencia computacional (C++/CUDA backend), se prescribe el siguiente stack con justificación técnica rigurosa para cada componente:

\subsection{JAX: Motor de Computación Numérica}

\textbf{JAX 0.4.20} es la \textit{capa fundamental} del predictor. Proporciona:

\begin{itemize}
    \item \textbf{XLA (Accelerated Linear Algebra)}: Compilación JIT a kernels optimizados para CPU/GPU/TPU
    \item \textbf{Diferenciación Automática}: \texttt{jax.grad()}, \texttt{jax.jacfwd()}, \texttt{jax.hessian()} requeridos por cálculo de Malliavin y JKO
    \item \textbf{Vectorización Automática}: \texttt{vmap()} para paralelización sin bucles explícitos
    \item \textbf{Composabilidad}: Transformaciones apilables (\texttt{jit(vmap(grad(...)))})
\end{itemize}

\textbf{Decisión de Diseño:} JAX es \textit{obligatorio} porque el Orquestador JKO requiere gradientes de Wasserstein (cálculo variacional) y las Ramas B/C requieren backpropagation a través de ecuaciones diferenciales.

\subsection{Equinox 0.11.2: Framework Neuronal Pythonico}

\textbf{Equinox} es el framework de redes neuronales para Rama B (Deep Galerkin Method) y Rama C (Neural ODEs). Seleccionado por:

\begin{enumerate}
    \item \textbf{Puramente Funcional}: Compatible con transformaciones JAX (\texttt{jit}, \texttt{vmap}, \texttt{grad})
    
    \item \textbf{Pytrees Nativos}: Los modelos son pytrees de JAX, no requieren conversión manual
    
    \item \textbf{Minimal Magic}: Sin estados ocultos, sin registros globales, sin metaclases
    
    \item \textbf{Diferenciabilidad Total}: Pesos de redes son arrays JAX puros → gradientes directos sin wrappers
    
    \item \textbf{Filtrado de Parámetros}: \texttt{eqx.filter()} permite separar parámetros entrenables de hiperparámetros estáticos
\end{enumerate}

\textbf{Ejemplo Técnico (Rama B - DGM):}
...
class DGM_HJB_Solver(eqx.Module):
    """
    Deep Galerkin Method para resolver HJB: V_t + H(x, V_x, V_xx) = 0
    """
    mlp: eqx.nn.MLP  # Red neuronal como pytree JAX
    
    def __init__(self, in_size, key):
        # MLP(in_size=d+1, out_size=1) donde d=dimension espacial, +1=tiempo
        self.mlp = eqx.nn.MLP(
            in_size=in_size, 
            out_size=1, 
            width_size=64,   # Capas ocultas de 64 neuronas
            depth=4,         # 4 capas ocultas
            key=key,
            activation=jax.nn.tanh  # Activación suave para derivadas
        )
    
    def __call__(self, t, x):
        """V(t, x) → escalar"""
        tx = jnp.concatenate([jnp.atleast_1d(t), x])
        return self.mlp(tx)[0]

# Uso: Diferenciación automática sin conversiones
def compute_residual(model, t, x):
    """Residual HJB: V_t + H(x, V_x, V_xx)"""
    v_t = jax.grad(lambda _t: model(_t, x))(t)       # Derivada temporal
    v_x = jax.grad(lambda _x: model(t, _x))(x)       # Gradiente espacial
    v_xx = jax.hessian(lambda _x: model(t, _x))(x)   # Hessiano espacial
    
    return v_t + hamiltonian(x, v_x, v_xx)  # Directo, sin abstracciones
\end{lstlisting}

\textbf{Alternativas Descartadas:}

\begin{itemize}
    \item \textbf{Flax}: Requiere estado mutable (\texttt{variables} dict), incompatible con \texttt{jit} puro
    \item \textbf{Haiku}: Usa \texttt{transform()} que rompe composabilidad con \texttt{vmap}
    \item \textbf{PyTorch}: No compilable a XLA, incompatible con JAX AD
\end{itemize}

\subsection{Diffrax 0.4.1: Solver de Ecuaciones Diferenciales Estocásticas}

\textbf{Diffrax} es el motor de integración para Rama C (Neural ODEs y SDEs). Seleccionado por:

\begin{enumerate}
    \item \textbf{Nativamente JAX}: Todos los solvers son \texttt{@jit}-compilables
    
    \item \textbf{Diferenciable End-to-End}: Backpropagation a través de la integración (adjoint method)
    
    \item \textbf{Soporte SDE Completo}: Integrales de Itô y Stratonovich con control adaptativo
    
    \item \textbf{Control de Error}: Solvers adaptativos (Heun, Dopri5, Tsit5) con tolerancia configurable
    
    \item \textbf{Compatibilidad Equinox}: ODEs parametrizados por \texttt{eqx.Module} sin conversión
\end{enumerate}

\textbf{Ejemplo Técnico (Rama C - Neural SDE):}
...
class NeuralSDE(eqx.Module):
    """
    SDE parametrizado: dX_t = f_θ(X_t)dt + g_θ(X_t)dW_t
    """
    drift_net: eqx.nn.MLP   # f_θ: Drift
    diffusion_net: eqx.nn.MLP  # g_θ: Volatilidad
    
    def __init__(self, state_size, key):
        key1, key2 = jax.random.split(key)
        self.drift_net = eqx.nn.MLP(state_size, state_size, width_size=32, depth=2, key=key1)
        self.diffusion_net = eqx.nn.MLP(state_size, state_size, width_size=32, depth=2, key=key2)
...
def solve_neural_sde(model, x0, t0, t1, brownian_key):
    """
    Integra SDE con solver adaptativo Heun (orden 1.5 para SDEs).
    Resultado es DIFERENCIABLE respecto a pesos de model.
    """
    # Término de drift (determinista)
    drift_term = dfx.ODETerm(model.drift)
    
    # Término de difusión (estocástico)
    brownian_motion = dfx.VirtualBrownianTree(
        t0=t0, 
        t1=t1, 
        tol=1e-3,  # Tolerancia de interpolación
        shape=(x0.shape[0],), 
        key=brownian_key
    )
    diffusion_term = dfx.ControlTerm(model.diffusion, brownian_motion)
    
    # Solver SDE adaptativo
    solver = dfx.Heun()  # Método de Heun para SDEs (orden 1.5 fuerte)
    
    # Integración con control de error
    solution = dfx.diffeqsolve(
        terms=dfx.MultiTerm(drift_term, diffusion_term),
        solver=solver,
        t0=t0,
        t1=t1,
        dt0=0.01,  # Paso inicial
        y0=x0,
        stepsize_controller=dfx.PIDController(atol=1e-5, rtol=1e-3),
        saveat=dfx.SaveAt(ts=jnp.linspace(t0, t1, 100))  # Guardar trayectoria
    )
    
    return solution.ys  # Array [100, state_size] diferenciable

# CRUCIALMENTE: Podemos calcular gradientes a través de la integración
@jax.jit
def loss_neural_sde(model, x0, target, brownian_key):
    """Loss = ||X_T - target||^2"""
    trajectory = solve_neural_sde(model, x0, t0=0.0, t1=1.0, brownian_key=brownian_key)
    x_final = trajectory[-1]
    return jnp.sum((x_final - target)**2)

# Backpropagation a través del SDE
grad_loss = jax.grad(loss_neural_sde)(model, x0, target, key)
\end{lstlisting}

\textbf{Ventajas sobre Alternativas:}

\begin{itemize}
    \item \textbf{vs. torchdiffeq (PyTorch)}: Diffrax compila a XLA (10-50x más rápido en GPU)
    \item \textbf{vs. scipy.integrate.solve\_ivp}: Diffrax es diferenciable (scipy no)
    \item \textbf{vs. JAX manual (Euler-Maruyama)}: Diffrax tiene control adaptativo de paso (crítico para estabilidad)
\end{itemize}

\textbf{Caso de Uso en Rama C:}

El Kernel C (Neural ODE) modela la dinámica de precio como:

\[
dX_t = f_\theta(X_t, t) dt + \sigma(X_t, t) dW_t
\]

donde $f_\theta$ es una red neuronal (Equinox) y la integración se realiza con Diffrax. El Orquestador JKO optimiza $\theta$ minimizando la divergencia de Wasserstein entre la distribución predicha y la observada.

\subsection{Signax 0.1.4: Cálculo de Log-Signatures}

\textbf{Signax} implementa el cálculo de signatures y log-signatures en álgebra de Lie, requerido por Rama D. Características críticas:

\begin{itemize}
    \item \textbf{GPU-Nativo}: Kernels CUDA optimizados para paths largos ($N > 1000$)
    \item \textbf{Diferenciable}: Gradientes exactos vía backpropagation (no aproximaciones)
    \item \textbf{Profundidad Configurable}: Soporta truncamiento a orden $M \in \{2, 3, 5\}$ (para load shedding)
    \item \textbf{Log-Domain}: Calcula en espacio de Lie (numéricamente estable vs signature tensor)
\end{itemize}

\textbf{Ejemplo Técnico (Rama D):}

\begin{lstlisting}[language=Python]
import signax

def compute_log_signature(path, depth):
    """
    path: Array [N, d] (N pasos temporales, d dimensiones)
    depth: Profundidad M de la signature (típicamente 3 o 5)
    
    Returns: Log-signature en álgebra de Lie (vector de dim O(d^M))
    """
    logsig = signax.logsignature(path, depth=depth)
    return logsig

# Diferenciación automática a través de signature
def signature_loss(path, target_logsig, depth):
    pred_logsig = signax.logsignature(path, depth=depth)
    return jnp.sum((pred_logsig - target_logsig)**2)

grad_signature = jax.grad(signature_loss)(path, target, depth)
\end{lstlisting}

\subsection{OTT-JAX 0.4.5: Transporte Óptimo Diferenciable}

\textbf{OTT-JAX} implementa el algoritmo de Sinkhorn estabilizado en log-domain para el Orquestador JKO. Seleccionado por:

\begin{itemize}
    \item \textbf{Estabilidad Numérica}: Log-Sinkhorn previene overflow/underflow en iteraciones largas
    \item \textbf{Regularización Entrópica}: Control fino de $\epsilon$ (crucial para convergencia JKO)
    \item \textbf{Gradientes Implícitos}: Backpropagation eficiente a través de Sinkhorn (método adjunto)
    \item \textbf{Geometrías Flexibles}: Soporta costos custom (ej. distancia de firmas)
\end{itemize}

\subsection{Stack Completo: Interoperabilidad y Flujo de Ejecución}

\begin{table}[h]
\centering
\begin{tabular}{|l|l|l|l|}
\hline
\textbf{Componente} & \textbf{Librería} & \textbf{Versión} & \textbf{Rol} \\
\hline
Computación base & JAX & 0.4.20 & XLA, AD, vmap, jit \\
Redes neuronales & Equinox & 0.11.2 & DGM (Rama B), parametrización \\
Solvers SDE/ODE & Diffrax & 0.4.1 & Neural ODEs (Rama C) \\
Signatures & Signax & 0.1.4 & Log-signatures (Rama D) \\
Transporte Óptimo & OTT-JAX & 0.4.5 & Orquestador JKO \\
Wavelets & PyWavelets & 1.4.1 & WTMM (SIA, CPU callback) \\
\hline
\end{tabular}
\end{table}

\textbf{Flujo de Ejecución Típico:}

\begin{enumerate}
    \item \textbf{Entrada}: Serie temporal $\{y_t\}$ ingresa al sistema
    
    \item \textbf{SIA (PyWavelets)}: Calcula WTMM → exponente Hölder $H_t$ (CPU callback)
    
    \item \textbf{Rama A}: Kernels estocásticos puros (JAX puro)
    
    \item \textbf{Rama B (Equinox)}: DGM-HJB solver → predicción PDE
    
    \item \textbf{Rama C (Diffrax + Equinox)}: Neural SDE → trayectoria predicha
    
    \item \textbf{Rama D (Signax)}: Log-signature del path → features topológicas
    
    \item \textbf{Orquestador (OTT-JAX)}: JKO step → pesos óptimos $\rho^*$
    
    \item \textbf{Salida}: Predicción fusionada $\hat{y}_{t+1}$
\end{enumerate}

\textbf{Ventaja Crítica de este Stack:}

\begin{itemize}
    \item \textbf{Todo compilable a XLA}: Pipeline completo se convierte en un único kernel GPU
    \item \textbf{Diferenciable end-to-end}: Gradientes fluyen desde pérdida final hasta entrada
    \item \textbf{Sin conversiones tipo}: Todos los componentes hablan "pytrees de JAX"
    \item \textbf{Reproducible}: Versiones ancladas garantizan comportamiento idéntico en 5 años
\end{itemize}

\textbf{Conclusión de Selección:}

Este stack NO es arbitrario. Fue seleccionado porque:

\begin{enumerate}
    \item Equinox + Diffrax son los \textit{únicos} frameworks que soportan backpropagation a través de SDEs en JAX puro
    
    \item Signax es la \textit{única} librería de signatures nativa en JAX con soporte GPU
    
    \item OTT-JAX es la implementación \textit{estándar de facto} de Sinkhorn diferenciable en el ecosistema JAX
    
    \item Todas las librerías comparten la misma filosofía: \textit{funciones puras, pytrees, JIT-compilable}
\end{enumerate}

Introducir librerías "por la puerta trasera" (ej. PyTorch para Rama B, torchdiffeq para Rama C) rompería:

\begin{itemize}
    \item Compilabilidad XLA (no se puede \texttt{jit} código PyTorch)
    \item Diferenciabilidad (AD de JAX no atraviesa tensores de PyTorch)
    \item Reproducibilidad (mezclar backends introduce no-determinismo)
\end{itemize}

Por lo tanto, este stack está \textbf{grabado en piedra} en el diseño del predictor.
...
\section{Arquitectura Física de Directorios (Restricción de Implementación)}

Para garantizar la estricta Separación de Responsabilidades (Separation of Concerns) y proteger la pureza de las funciones compiladas por XLA, cualquier repositorio de código derivado de esta especificación \textbf{debe} instanciarse respetando la siguiente jerarquía física de paquetes:

\begin{itemize}
    \item \texttt{stochastic\_predictor/api/}: Frontera de contrato externo (Facade). Debe contener exclusivamente la configuración global (\texttt{PredictorConfig}), los esquemas de validación estricta usando tipos de datos seguros, y la lógica de degradación de inferencia (Load Shedding).
    
    \item \texttt{stochastic\_predictor/core/}: Capa de orquestación central. Aloja el bucle principal de transporte JKO, la regularización de Sinkhorn y el monitoreo de entropía/curtosis. No interactúa directamente con los sockets de I/O.
    
    \item \texttt{stochastic\_predictor/kernels/}: Motores matemáticos puros en XLA (SIA, DGM, Itô/Lévy, Signatures). Todos sus submódulos deben exponer funciones puras (\textit{stateless}) para maximizar la velocidad de compilación JIT y paralelización vectorizada (\texttt{vmap}).
    
    \item \texttt{stochastic\_predictor/io/}: Gestión de Entrada/Salida física. Responsable de la sanitización de señales entrantes y de ejecutar el motor asíncrono para el Snapshotting Atómico de memoria latente.
    
    \item \texttt{tests/}: Suite de validación externa, separada del paquete principal, destinada a las pruebas causales y de consistencia de bit (Hardware Parity).
\end{itemize}

\section{Especificación Estricta del Entorno (Dependency Pinning)}

Debido a que JAX actúa como un compilador \textit{Just-In-Time} (JIT) sobre hardware acelerado, variaciones menores en las versiones de las librerías pueden alterar heurísticas de fusión de grafos, rompiendo la consistencia de bit matemática. El entorno de ejecución queda \textbf{congelado} a la siguiente matriz de compatibilidad (Golden Master):

\begin{lstlisting}[language=bash, caption={Golden Master Environment Specification}]
# --- CORE MATHEMATICS & HARDWARE (JAX/XLA) ---
jax==0.4.20
jaxlib==0.4.20+cuda12.cudnn89

# --- NEURAL ODEs & STOCHASTIC SOLVERS ---
equinox==0.11.2
diffrax==0.4.1
jaxtyping==0.2.24

# --- OPTIMAL TRANSPORT (JKO Flow) ---
ott-jax==0.4.5

# --- ROUGH PATHS & TOPOLOGY ---
signax==0.1.4

# --- SIGNAL PROCESSING (WTMM) ---
PyWavelets==1.4.1
scipy==1.11.4
...
\noindent \textbf{Prohibición de Resolución Dinámica}

Queda estrictamente prohibido el uso de operadores de versión abierta (ej. \texttt{>=}) o comandos de actualización dinámica (ej. \texttt{pip install -U}) en los entornos de Integración Continua (CI/CD) o Producción.
...
\section{Anclaje Estricto de Dependencias (Dependency Pinning)}

\textbf{Problema Crítico de Evolución del Ecosistema:}

El ecosistema de JAX y sus dependencias científicas evoluciona a \textbf{velocidad vertiginosa}. Una actualización menor de librerías puede alterar sutilmente cómo el compilador XLA fusiona operaciones, reorganiza reducciones, o maneja precisión de punto flotante, \textbf{rompiendo nuestras garantías de consistencia bit-exact}.

\textbf{Ejemplos de Cambios No-Triviales en Actualizaciones Menores:}

\begin{itemize}
    \item \textbf{JAX 0.4.19 → 0.4.20}: Cambio en orden de fusión de kernels en \texttt{jnp.einsum} afecta reducciones acumulativas en Rama D (Signatures)
    
    \item \textbf{OTT-JAX 0.4.4 → 0.4.5}: Modificación en algoritmo de Sinkhorn (número de iteraciones por defecto) altera convergencia del Orquestador JKO
    
    \item \textbf{cuDNN 8.8 → 8.9}: Cambio en TensorCore precision handling para \texttt{matmul} rompe paridad CPU/GPU en tests
    
    \item \textbf{Signax 0.1.3 → 0.1.4}: Refactorización interna de cálculo de log-signatures cambia representación de álgebra de Lie
    
    \item \textbf{Equinox 0.11.2 → 0.11.3}: Ajuste en inicialización de pesos de redes neuronales (Rama B DGM) cambia valores de predicción
\end{itemize}

\textbf{Consecuencia sin Anclaje:}

\begin{enumerate}
    \item \textbf{Tests de Regresión Fallan}: El código que pasaba tests hace 6 meses ahora falla con \texttt{AssertionError: max difference = 3.2e-4} (vs tolerancia 1e-7)
    
    \item \textbf{Snapshots Incompatibles}: Un predictor entrenado con JAX 0.4.19 no puede restaurarse correctamente en JAX 0.4.21 (estructura de pesos diferentes)
    
    \item \textbf{Resultados No-Reproducibles}: Papers científicos que citan resultados experimentales no pueden replicarse años después
    
    \item \textbf{Divergencia en Producción}: Actualizaciones automáticas de Docker base images rompen sistemas en producción sin previo aviso
    
    \item \textbf{Debugging Imposible}: Comparar logs de hace 3 meses es inútil si el entorno cambió
\end{enumerate}

\textbf{Solución: Dependency Pinning con Versiones Exactas}

Definir archivos de entorno con \textbf{versiones congeladas} de todas las dependencias críticas. Esto garantiza que el código se comporte de manera \textit{matemáticamente idéntica} dentro de 5 años.

\subsection{Archivo: requirements.txt (Pip)}

\begin{lstlisting}[language=bash]
# requirements.txt - Universal Predictor
# Generado: 18 de febrero de 2026
# Python: 3.10.12 (OBLIGATORIO, no usar 3.11+ por cambios en RNG)

# ═══════════════════════════════════════
# CORE DEPENDENCIES (JAX Stack)
# ═══════════════════════════════════════
jax==0.4.20
jaxlib==0.4.20+cuda12.cudnn89  # CUDA 12.1, cuDNN 8.9

# NOTA CRÍTICA: jaxlib debe coincidir EXACTAMENTE con versión CUDA/cuDNN del host
# Alternativas según infraestructura:
#   - CPU only: jaxlib==0.4.20
#   - CUDA 11.8: jaxlib==0.4.20+cuda11.cudnn86
#   - TPU: jaxlib==0.4.20+tpu

# ═══════════════════════════════════════
# SCIENTIFIC STACK (OT, Signatures, ODEs)
# ═══════════════════════════════════════
ott-jax==0.4.5           # Optimal Transport (Orquestador JKO)
signax==0.1.4            # Log-Signatures (Rama D)
equinox==0.11.2          # Neural ODEs (Rama C)
diffrax==0.4.1           # SDE/ODE solvers (Rama C)

# ═══════════════════════════════════════
# NUMERICAL & TYPING
# ═══════════════════════════════════════
jaxtyping==0.2.25        # Type annotations (Float[Array, "n m"])
numpy==1.24.3            # OBLIGATORIO: 1.24.x (1.25+ rompe compatibilidad dtype)
scipy==1.11.4            # Funciones especiales (Besov wavelets)

# ═══════════════════════════════════════
# PERSISTENCE & SERIALIZATION
# ═══════════════════════════════════════
msgpack==1.0.7           # Snapshots binarios (más rápido que pickle)
msgpack-numpy==0.4.8     # Serialización de arrays numpy

# ═══════════════════════════════════════
# MONITORING & LOGGING
# ═══════════════════════════════════════
prometheus-client==0.19.0  # Métricas (jitter, latencia, etc.)
# pynvml==11.5.0           # GPU monitoring (opcional)
...
# ═══════════════════════════════════════
# Ejecutar: pip freeze > requirements-freeze.txt
# Verificar: sha256sum requirements-freeze.txt
# Hash esperado (ejemplo): 
#   a7f3b2c9d1e4f5g6h7i8j9k0l1m2n3o4p5q6r7s8t9u0v1w2x3y4z5
\end{lstlisting}

\subsection{Archivo: environment.yml (Conda/Mamba)}

\begin{lstlisting}[language=bash]
# environment.yml - Universal Predictor
# Generado: 18 de febrero de 2026
# Uso: conda env create -f environment.yml

name: universal-predictor-v1.0
channels:
  - conda-forge
  - nvidia  # Para CUDA/cuDNN
  - defaults
...
dependencies:
  ...
  # ═══════════════════════════════════════
  - python=3.10.12  # NO usar 3.11+ (cambios en random.seed())
  ...
  # ═══════════════════════════════════════
  # PIP DEPENDENCIES (JAX stack no disponible en conda)
  # ═══════════════════════════════════════
  - pip:
    - jax==0.4.20
    - jaxlib==0.4.20+cuda12.cudnn89
...
# ═══════════════════════════════════════
# conda activate universal-predictor-v1.0
# python -c "import jax; print(jax.__version__)"  # Debe ser 0.4.20
# python -c "import jax; print(jax.devices())"    # Debe mostrar GPU
# python -c "import ott; print(ott.__version__)"  # Debe ser 0.4.5
\end{lstlisting}

\subsection{Dockerfile con Versiones Ancladas}

\begin{lstlisting}[language=bash]
# Dockerfile - Universal Predictor
# Imagen base: NVIDIA CUDA 12.1, cuDNN 8.9, Ubuntu 22.04
FROM nvidia/cuda:12.1.0-cudnn8-runtime-ubuntu22.04

# Metadatos
LABEL maintainer="predictor-team@example.com"
LABEL version="1.0.0"
LABEL description="Universal Stochastic Predictor with pinned dependencies"

# Variables de entorno (reproducibilidad)
ENV DEBIAN_FRONTEND=noninteractive
ENV PYTHONUNBUFFERED=1
ENV PYTHONDONTWRITEBYTECODE=1
ENV CUDA_VISIBLE_DEVICES=0

# Instalar Python 3.10.12 EXACTO
RUN apt-get update && apt-get install -y \
    software-properties-common \
    && add-apt-repository ppa:deadsnakes/ppa \
    && apt-get update \
    && apt-get install -y python3.10=3.10.12-1+jammy1 \
    python3.10-dev \
...
# Anclar pip a versión específica
RUN wget https://bootstrap.pypa.io/get-pip.py \
    && python3.10 get-pip.py pip==23.3.1 \
    && rm get-pip.py

# Copiar requirements.txt con versiones congeladas
COPY requirements.txt /app/requirements.txt
WORKDIR /app

# Instalar dependencias (sin actualizar nada)
RUN pip install --no-cache-dir --no-deps -r requirements.txt

# Verificar instalación
RUN python3.10 -c "import jax; assert jax.__version__ == '0.4.20', 'JAX version mismatch'"
RUN python3.10 -c "import ott; assert ott.__version__ == '0.4.5', 'OTT version mismatch'"
RUN python3.10 -c "import signax; assert signax.__version__ == '0.1.4', 'Signax version mismatch'"

# Copiar código del predictor
COPY stochastic_predictor/ /app/stochastic_predictor/
COPY tests/ /app/tests/

# Health check
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD python3.10 -c "import jax; jax.devices()"

# Comando por defecto
CMD ["python3.10", "-m", "stochastic_predictor.main"]
\end{lstlisting}

\subsection{Protocolo de Actualización Controlada}

Si eventualmente se requiere actualizar dependencias (ej. parche de seguridad en JAX), seguir protocolo estricto:

\begin{enumerate}
    \item \textbf{Crear Rama de Testing}: \texttt{git checkout -b deps/jax-0.4.21-evaluation}
    
    \item \textbf{Actualizar SOLO la Dependencia Candidata}:
    \begin{lstlisting}[language=bash]
# requirements-test.txt
jax==0.4.21  # <-- Solo esta línea cambia
jaxlib==0.4.21+cuda12.cudnn89
# Resto idéntico a requirements.txt
    \end{lstlisting}
    
    \item \textbf{Ejecutar Suite Completa de Tests}:
    \begin{lstlisting}[language=bash]
pytest tests/ --cov=stochastic_predictor --cov-report=html -v
    \end{lstlisting}
    
    \item \textbf{Validar Consistencia Bit-Exact}:
    \begin{lstlisting}[language=Python]
# tests/test_version_migration.py
def test_jax_0_4_21_compatibility():
    """Valida que JAX 0.4.21 produce resultados idénticos a 0.4.20"""
    # Cargar snapshot de referencia (generado con JAX 0.4.20)
    reference = load_snapshot("tests/fixtures/reference_jax_0.4.20.pkl")
    
    # Ejecutar con nueva versión
    predictor = UniversalPredictor(config)
    result = predictor.predict_next(reference.observation)
    
    # Comparación bit-exact
    assert jnp.allclose(
        result.predicted_next,
        reference.predicted_next,
        atol=1e-14,  # Tolerancia máxima para float64
        rtol=1e-13
    ), "JAX 0.4.21 produce resultados diferentes a 0.4.20"
    \end{lstlisting}
    
    \item \textbf{Benchmark de Performance}:
    \begin{lstlisting}[language=bash]
python benchmarks/latency_comparison.py \
    --old-env requirements.txt \
    --new-env requirements-test.txt \
    --iterations 1000
    \end{lstlisting}
    
    \item \textbf{Validación en Staging} (30 días mínimo):
    \begin{itemize}
        \item Desplegar versión nueva en ambiente de staging
        \item Comparar métricas de producción vs staging (lado a lado)
        \item Monitorear jitter, latencia P99, violaciones de SLA
    \end{itemize}
    
    \item \textbf{Aprobación por Comité Técnico}:
    \begin{itemize}
        \item Revisar diff de resultados de tests
        \item Aprobar cambios de performance (si degradan <5\%)
        \item Documentar breaking changes (si existen)
    \end{itemize}
    
    \item \textbf{Merge a Main con Tag de Versión}:
    \begin{lstlisting}[language=bash]
git merge deps/jax-0.4.21-evaluation
git tag -a v1.1.0-jax-0.4.21 -m "Upgrade JAX 0.4.20 -> 0.4.21"
git push origin v1.1.0-jax-0.4.21
    \end{lstlisting}
\end{enumerate}

\textbf{Casos Donde NO Actualizar:}

\begin{itemize}
    \item \textbf{Nueva versión rompe tests}: Si algún test falla, \textit{rechazar actualización}
    \item \textbf{Performance degrada >5\%}: Si latencia P99 aumenta, \textit{rechazar}
    \item \textbf{Resultados divergen >1e-6}: Si predicciones cambian fuera de tolerancia, \textit{rechazar}
    \item \textbf{No hay parche de seguridad crítico}: Si solo son "nuevas features", \textit{no actualizar}
\end{itemize}

\textbf{Justificación Matemática del Anclaje:}

Un predictor estocástico entrenado con Orquestador JKO converge a:

\[
\rho^*_{t+1} = \arg\min_{\rho \in \mathcal{P}(\mathbb{R}^d)} \left\{ \mathcal{E}(\rho) + \frac{1}{2\tau} W_2^2(\rho, \rho_t) \right\}
\]

donde $W_2$ es la distancia de Wasserstein (calculada por OTT-JAX via Sinkhorn). Si OTT-JAX cambia:

\begin{itemize}
    \item \textbf{Número de iteraciones de Sinkhorn}: $\rho^*$ converge a punto diferente
    \item \textbf{Regularización entrópica $\epsilon$}: Gradientes del Orquestador cambian
    \item \textbf{Inicialización de dual variables}: Trayectoria de optimización diverge
\end{itemize}

Similarmente, las log-signatures de Rama D dependen de:

\[
\text{Sig}(X)_{1,t} = X_t - X_1 + \frac{1}{2} \int_1^t (X_s - X_1) \otimes dX_s + \ldots
\]

Si Signax cambia el cálculo de la integral de Stratonovich, la representación en álgebra de Lie cambia, y los pesos del Orquestador entrenados previamente quedan obsoletos.

\textbf{Conclusión:}

El \textbf{Anclaje Estricto de Dependencias} NO es opcional. Es la \textit{capa base} sobre la cual se ejecuta todo el sistema. Sin versiones congeladas:

\begin{itemize}
    \item Tests de regresión son inútiles (fallan aleatoriamente)
    \item Snapshots de producción se corrompen entre actualizaciones
    \item Debugging es imposible (entorno cambia sin control)
    \item Papers científicos no son reproducibles
    \item Compliance regulatorio falla (finanzas requieren auditoría de código)
\end{itemize}

Con anclaje estricto:

\begin{itemize}
    \item Código de hoy funciona idénticamente en 5 años
    \item Tests de bit-exact parity pasan consistentemente
    \item Snapshots son compatibles a través del tiempo
    \item Actualizaciones son controladas, validadas, y documentadas
    \item Auditorías pueden replicar resultados históricos exactos
\end{itemize}

El overhead de mantener \texttt{requirements.txt} congelado es \textit{cero} comparado con el \textit{riesgo catastrófico} de dependencias flotantes.
