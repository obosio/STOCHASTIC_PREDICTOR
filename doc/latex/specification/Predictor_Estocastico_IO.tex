...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item \textbf{Detección de Señales Congeladas (Frozen Signal Validation):} Si el stream inyecta exactamente el mismo valor durante $N_{freeze} \geq 5$ pasos consecutivos, el sistema debe:
        \begin{enumerate}
            \item Calcular la varianza de los últimos $N_{freeze}$ valores: $\text{Var}([y_{t-4}, y_{t-3}, y_{t-2}, y_{t-1}, y_t]) = 0$
            \item Identificar esto como falla de sensor o corrupción de fuente de datos
            \item Emitir \texttt{FrozenSignalAlarmEvent} con timestamp del evento
            \item \textbf{Impacto matemático:} El exponente de Hölder en Rama D requiere variabilidad: $H_t = \lim_{s \to 0} \frac{\log |\gamma(t+s) - \gamma(t)|}{\log s}$. Con señal congelada, el numerador es cero, causando singularidades o indeterminaciones ($0/\log s$). Esto invalida el espectro multifractal. El sistema debe:
            \begin{itemize}
                \item Congelar la Rama Topológica (Kernel D) en último valor válido
                \item NO actualizar pesos del orquestador (mantener inercia)
                \item Activar modo de inferencia degradada
                \item Continuar predicciones usando Ramas A, B, C solamente
            \end{itemize}
            \item \textbf{Recuperación:} Una vez que se detecte varianza $> 0.1 \times \text{Var}_{histórica}$ durante 2 pasos consecutivos, liberar el bloqueo de Rama D y reanudar normalidad.
        \end{enumerate}
    \end{itemize}
...
\section{Políticas de Seguridad en la Capa de I/O (Credenciales)}

El diseño del Predictor Estocástico exige operaciones de ingesta de alta frecuencia contra infraestructuras de mercado externas (ej. WebSockets institucionales, brokers o APIs REST). El acceso a estas infraestructuras introduce vectores de vulnerabilidad crítica.

\vspace{0.5em}

\noindent \textbf{Inyección de Entorno Segura}

Queda categóricamente prohibido el \textit{hardcoding} (codificación en duro) de tokens, claves API, secretos de bases de datos o credenciales de conexión en cualquier módulo del código fuente futuro (ej. \texttt{io/streams.py}). Toda implementación \textbf{debe} aplicar el patrón de Inyección de Entorno, leyendo estas credenciales en tiempo de ejecución desde variables del sistema operativo o archivos locales \texttt{.env}.

\vspace{0.5em}

\noindent \textbf{Exclusión de Control de Versiones}

El repositorio de implementación resultante debe incluir reglas explícitas de exclusión en su sistema de control de versiones (ej. forzar \texttt{*.env} en el archivo \texttt{.gitignore}) para garantizar que ningún secreto sea expuesto inadvertidamente.
