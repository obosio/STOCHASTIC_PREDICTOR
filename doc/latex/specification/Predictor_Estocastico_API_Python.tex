...
\subsection{Entrada Operativa (\texorpdfstring{$y_t, y_{reference}, \tau$}{y\_t, y\_reference, tau})}
\begin{lstlisting}[language=Python]
@dataclass(frozen=True)
class ProcessState:
    magnitude: Float[Array, "1"]  # y_t (Normalizado o Absoluto)
    reference: Float[Array, "1"]  # y_reference (Magnitud de referencia)
...
\section{Apagado Elegante (Graceful Shutdown) para Contenedores}

\textbf{Problema en Entornos Orquestados (Docker/Kubernetes):}

En infraestructura de producción moderna, el predictor opera típicamente en contenedores que pueden ser:

\begin{itemize}
    \item \textbf{Reiniciados}: Por actualizaciones de imagen, cambios de configuración, o healthcheck failures
    \item \textbf{Migrados}: Rebalanceo de carga entre nodos del cluster (rolling updates, node draining)
    \item \textbf{Escalados}: Auto-scaling que crea/destruye pods según demanda
    \item \textbf{Terminados}: Reclamación de recursos o fallos de infraestructura
\end{itemize}

En todos estos casos, Kubernetes/Docker envía \textbf{SIGTERM} (señal POSIX 15) al proceso principal, dándole \textit{grace period} (típicamente 30 segundos) antes de enviar \textbf{SIGKILL} (forzado, no capturado).

\textbf{Consecuencia de Apagado Abrupto (sin manejo de SIGTERM):}

\begin{enumerate}
    \item \textbf{Pérdida de Inercia del Orquestador}: Los pesos $\rho_i$ entrenados se pierden si no están en snapshot reciente
    \item \textbf{Corrupción de Buffer SIA}: El buffer circular de WTMM queda truncado, perdiendo contexto histórico
    \item \textbf{Estado CUSUM Inconsistente}: Las variables $C^+$, $C^-$ no reflejan el régimen real al reiniciar
    \item \textbf{Snapshots Parciales}: Si un \texttt{save\_snapshot\_nonblocking()} estaba en vuelo, el archivo puede corromperse
    \item \textbf{Datos en Buffer Residual}: Observaciones recibidas pero no procesadas se descartan
\end{enumerate}

Resultado: El sistema reinicia en \textit{estado frío}, perdiendo hasta 30 minutos de aprendizaje adaptativo (dependiendo del intervalo de snapshots).

\textbf{Solución: Captura de SIGTERM con Protocolo de Shutdown Ordenado}
...
class UniversalPredictor_GracefulShutdown:
    def __init__(self, config: PredictorConfig):
        ...
        self.residual_buffer = []  # Buffer de observaciones no procesadas
        
        # Registro de señales POSIX
        signal.signal(signal.SIGTERM, self._handle_sigterm)
        signal.signal(signal.SIGINT, self._handle_sigint)  # Ctrl+C en desarrollo
    ...
    def _handle_sigterm(self, signum, frame):
        """
        Manejador de SIGTERM (enviado por Kubernetes/Docker).
        Inicia protocolo de apagado elegante.
        """
    ...
    def _handle_sigint(self, signum, frame):
        """
        Manejador de SIGINT (Ctrl+C local, desarrollo).
        """
        self.logger.warning(f"[SIGINT] Received signal {signum}. Initiating graceful shutdown...")
        self.shutdown_requested.set()
    
    def accept_observation(self, obs: MarketObservation) -> Optional[PredictionResult]:
        """
        Método thread-safe para recibir observaciones.
        Rechaza nuevas entradas si shutdown está en curso.
        """
        ...
        # Procesamiento inmediato (o delegación a cola)
        return self._process_observation(obs)
    
    def _process_observation(self, obs: MarketObservation) -> PredictionResult:
        """
        Procesa observación y emite predicción.
        """
        result = self.predictor.predict_next(obs)
        
        with self.input_buffer_lock:
            # Remover de buffer residual ya que se procesó
    ...
    def graceful_shutdown(self, timeout_seconds: int = 25):
        """
        Protocolo de apagado elegante (llamado automáticamente por SIGTERM).
        
        Pasos:
        1. Bloquear nuevas observaciones
        2. Procesar buffer residual
        3. Esperar snapshots asíncronos pendientes
        4. Ejecutar snapshot final
        5. Cerrar recursos (sockets, threads)
        6. Salir con código 0
        
        Args:
            timeout_seconds: Tiempo máximo antes de rendirse (debe ser < grace period de K8s)
        """
        ...
        # PASO 1: Bloquear nuevas entradas
        self.logger.info("[1/5] Blocking new observations...")
        self.is_accepting_data = False
        time.sleep(0.1)  # Dar tiempo a threads en vuelo para terminar
        self.logger.info("      └─ Input blocked: ✓")
        
        # PASO 2: Procesar buffer residual
        self.logger.info("[2/5] Processing residual buffer...")
        with self.input_buffer_lock:
            residual_count = len(self.residual_buffer)
            self.logger.info(f"      └─ Residual observations: {residual_count}")
            ...
            remaining = len(self.residual_buffer)
            if remaining > 0:
                self.logger.warning(f"      └─ {remaining} observations dropped (timeout)")
            else:
                self.logger.info("      └─ Residual buffer cleared: ✓")
        
        # PASO 3: Esperar snapshots asíncronos pendientes
        self.logger.info("[3/5] Waiting for pending async snapshots...")
        ...
        if pending_snapshot is not None and not pending_snapshot.done():
            try:
                ...
                self.logger.info("      └─ Async snapshot completed: ✓")
            except concurrent.futures.TimeoutError:
                self.logger.error("      └─ Async snapshot timed out (may be corrupted)")
            except Exception as e:
                self.logger.error(f"      └─ Async snapshot failed: {e}")
        else:
            self.logger.info("      └─ No pending snapshots: ✓")
        
        # PASO 4: Snapshot final (síncrono, garantizado)
        self.logger.info("[4/5] Executing final snapshot (synchronous)...")
        try:
            final_snapshot_path = f"snapshots/shutdown_{int(time.time())}.pkl"
            self.predictor.save_snapshot(final_snapshot_path)
            self.logger.info(f"      └─ Final snapshot saved: {final_snapshot_path}")
        except Exception as e:
            self.logger.error(f"      └─ Final snapshot FAILED: {e}")
            # Continuar de todos modos, no queremos bloquear el shutdown
        
        # PASO 5: Cerrar recursos
        self.logger.info("[5/5] Closing resources...")
        try:
            # Cerrar ThreadPoolExecutor (si existe)
            if hasattr(self.predictor, 'io_executor'):
                self.predictor.io_executor.shutdown(wait=True, cancel_futures=False)
                self.logger.info("      └─ ThreadPoolExecutor closed: ✓")
            
            # Cerrar sockets de mercado (si aplica)
            # self.market_socket.close()
            
            # Limpiar caché JAX (opcional)
            # jax.clear_caches()
        ...
        # Salir con código 0 (limpio)
        sys.exit(0)
    
    def run_forever(self):
        """
        Bucle principal del predictor.
        Monitorea shutdown_requested en cada iteración.
        """
        self.logger.info("[MAIN] Predictor running. Waiting for market data...")
        
        try:
            while not self.shutdown_requested.is_set():
                # Simular recepción de datos (en producción: socket.recv, kafka.poll, etc.)
                # observation = self.market_socket.recv()
                # result = self.accept_observation(observation)
                
                time.sleep(0.001)  # Evitar busy-wait
                
        except KeyboardInterrupt:
            # Capturado por _handle_sigint, pero por si acaso
            self.logger.warning("[MAIN] KeyboardInterrupt detected")
        
        # Shutdown solicitado
        self.graceful_shutdown(timeout_seconds=25)
\end{lstlisting}

\textbf{Integración con Kubernetes ConfigMap:}
...
kind: Deployment
metadata:
  name: universal-predictor
spec:
  replicas: 3
  template:
    spec:
      terminationGracePeriodSeconds: 30  # Tiempo para graceful_shutdown()
      containers:
      - name: predictor
        ...
        lifecycle:
          preStop:
            exec:
              # Kubernetes ejecuta esto ANTES de enviar SIGTERM
              # Opcional: Notificar load balancer que el pod está muriendo
...
\textbf{Script Principal (main.py):}

\begin{lstlisting}[language=Python]
#!/usr/bin/env python3
import logging
from stochastic_predictor import UniversalPredictor_GracefulShutdown, PredictorConfig
...
def main():
    ...
    logging.basicConfig(
        ...
        handlers=[
            ...
            logging.FileHandler("/var/log/predictor.log")
        ]
    )
    ...
    config = PredictorConfig.from_json("config.json")
    
    # Inicializar predictor con graceful shutdown
    predictor = UniversalPredictor_GracefulShutdown(config)
    
    # Ejecutar (bloqueante hasta SIGTERM)
    predictor.run_forever()
    
    # NUNCA llega aquí (sys.exit(0) en graceful_shutdown)

if __name__ == "__main__":
    main()
\end{lstlisting}

\textbf{Prueba de Apagado Elegante:}

\begin{lstlisting}[language=Python]
def test_graceful_shutdown_protocol():
    """
    Simula apagado elegante con buffer residual y snapshot pendiente.
    """
    import signal
    import os
    import time
    
    config = PredictorConfig(checkpoint_interval=100)
    predictor = UniversalPredictor_GracefulShutdown(config)
    
    # Simular carga de trabajo
    for i in range(50):
        obs = MarketObservation(
            price=jnp.array([float(i)]),
            target=jnp.array([0.0]),
            timestamp_ns=time.time_ns()
        )
        predictor.accept_observation(obs)
    
    # Disparar snapshot asíncrono
    predictor.predictor.save_snapshot_nonblocking("test_snapshot.pkl")
    
    # Añadir observaciones al buffer residual
    for i in range(10):
        obs = MarketObservation(
            price=jnp.array([float(100 + i)]),
            target=jnp.array([0.0]),
            timestamp_ns=time.time_ns()
        )
        with predictor.input_buffer_lock:
            predictor.residual_buffer.append(obs)
    
    # Simular SIGTERM (en test, llamamos directamente al handler)
    predictor._handle_sigterm(signal.SIGTERM, None)
    
    # Verificar que shutdown se ejecutó
    assert predictor.shutdown_requested.is_set(), "Shutdown not triggered"
    
    # Ejecutar shutdown manualmente (en producción es automático)
    try:
        predictor.graceful_shutdown(timeout_seconds=10)
    except SystemExit as e:
        assert e.code == 0, f"Exit code should be 0, got {e.code}"
    
    # Verificar que snapshot final existe
    final_snapshots = [f for f in os.listdir("snapshots") if f.startswith("shutdown_")]
    assert len(final_snapshots) > 0, "Final snapshot not created"
    
    print("[✓] Graceful shutdown test PASSED")
    print(f"    Final snapshot: {final_snapshots[-1]}")
    print(f"    Residual buffer: cleared")
    print(f"    Async snapshots: completed")
\end{lstlisting}

\textbf{Cronología de Shutdown (Típica):}

\begin{table}[h]
\centering
\begin{tabular}{|l|l|l|}
\hline
\textbf{Tiempo (s)} & \textbf{Evento} & \textbf{Acción} \\
\hline
$t=0.000$ & Kubernetes: \texttt{kubectl delete pod predictor-xyz} & Envía SIGTERM al PID 1 \\
$t=0.001$ & Python: \texttt{\_handle\_sigterm()} captura señal & Set \texttt{shutdown\_requested} \\
$t=0.002$ & Predictor: Bloquea nuevas observaciones & \texttt{is\_accepting\_data = False} \\
$t=0.010$ & Predictor: Procesa buffer residual (10 obs) & Loop \texttt{\_process\_observation()} \\
$t=0.500$ & Predictor: Espera snapshot asíncrono en vuelo & \texttt{future.result(timeout=20)} \\
$t=1.200$ & Predictor: Snapshot final (síncrono) & \texttt{save\_snapshot()} \\
$t=1.800$ & Predictor: Cierra ThreadPoolExecutor & \texttt{executor.shutdown(wait=True)} \\
$t=2.000$ & Python: \texttt{sys.exit(0)} & Proceso termina limpiamente \\
$t=2.001$ & Kubernetes: Recibe exit code 0 & Marca pod como \texttt{Completed} \\
\hline
\end{tabular}
\end{table}

\textbf{Comparación: Shutdown Abrupto vs Elegante}

\begin{table}[h]
\centering
\begin{tabular}{|l|c|c|}
\hline
\textbf{Métrica} & \textbf{Abrupto (SIGKILL)} & \textbf{Elegante (SIGTERM)} \\
\hline
...
\textbf{Configuración de Timeouts (Recomendaciones):}

\begin{itemize}
    \item \textbf{Kubernetes \texttt{terminationGracePeriodSeconds}}: 30 segundos (estándar)
    \item \textbf{Python \texttt{graceful\_shutdown(timeout\_seconds)}}: 25 segundos (margen de 5s)
    \item \textbf{Snapshot asíncrono \texttt{result(timeout)}}: 20 segundos (dentro del timeout de shutdown)
    \item \textbf{Buffer residual processing}: Máximo 10 segundos o 1000 observaciones
\end{itemize}

\textbf{Consideraciones de Producción:}

\begin{enumerate}
    \item \textbf{Snapshots Incrementales}: Si el estado es muy grande (>100 MB), considerar snapshots diferenciales para reducir tiempo de escritura
    
    \item \textbf{Health Probes}: Kubernetes debe tener \texttt{readinessProbe} que falle inmediatamente cuando \texttt{is\_accepting\_data = False}, para que el load balancer deje de enviar tráfico
    
    \item \textbf{Pre-Stop Hook}: Usar \texttt{lifecycle.preStop} para notificar a sistemas externos (ej. API Gateway) que el pod está en shutdown
    
    \item \textbf{Persistencia Distribuida}: Guardar snapshots finales en almacenamiento compartido (NFS, S3) en lugar de volumen local del pod
    
    \item \textbf{Logging Estructurado}: Todos los logs de shutdown deben ser JSON con campo \texttt{shutdown\_phase} para análisis post-mortem
    
    \item \textbf{Métricas de Shutdown}: Exponer contador Prometheus \texttt{predictor\_graceful\_shutdowns\_total} y histogram \texttt{predictor\_shutdown\_duration\_seconds}
\end{enumerate}

\textbf{Integración con Prometheus:}
...
class UniversalPredictor_GracefulShutdown_Monitored:
    ...
    def graceful_shutdown(self, timeout_seconds: int = 25):
        self.shutdown_counter.inc()
        
        with self.shutdown_duration.time():
            # ... (código de shutdown) ...
            
            with self.input_buffer_lock:
                self.residual_observations.observe(len(self.residual_buffer))
...
El \textbf{Graceful Shutdown} es \textit{obligatorio} para cualquier sistema stateful desplegado en contenedores. Sin captura de SIGTERM:

\begin{itemize}
    \item Rolling updates de Kubernetes destruyen el estado entrenado del Orquestador
    \item Node draining causa pérdida de 30+ minutos de adaptación a régimen actual
    \item Auto-scaling down elimina snapshots en vuelo, corrompiendo persistencia
    \item Exit code 137 (SIGKILL) indica fallo en logs, complicando debugging
\end{itemize}

Con el protocolo implementado:

\begin{itemize}
    \item 0\% pérdida de datos (buffer residual procesado)
    \item Estado completo persistido en <2 segundos
    \item Recuperación en <1 minuto (vs 30+ minutos cold start)
    \item Exit code 0 → logs limpios, métricas de salud correctas
    \item Compatible con rolling updates sin downtime de predicción
\end{itemize}

El overhead de implementación es \textit{mínimo} (100 líneas de código) comparado con el \textit{riesgo operacional masivo} de no implementarlo.
...
\section{Telemetría de Jitter (Gatillo Preciso para Load Shedding)}

\textbf{Problema de Precisión en Detección de Saturación:}

El Load Shedding descrito en la sección anterior utiliza \textbf{EWMA de latencia media} como gatillo. Sin embargo, existe un problema sutil:

\[
\text{EWMA}(\text{latencia}) = \alpha \cdot L_t + (1-\alpha) \cdot \text{EWMA}_{t-1}
\]

Esta métrica captura la \textit{tendencia central} de la latencia, pero \textbf{ignora la variabilidad} (jitter). En un sistema que procesa frames de mercado a frecuencia variable, la \textit{varianza} de la latencia es crítica:

\textbf{Escenario Problemático:}

\begin{itemize}
    \item Tick \#100: Latencia = 0.8ms (rápido, Rama D con $M=5$)
    \item Tick \#101: Latencia = 22ms (lento, GC de JAX o swap de memoria)
    \item Tick \#102: Latencia = 0.9ms (rápido nuevamente)
    \item Tick \#103: Latencia = 25ms (lento, conflicto de I/O)
    \item ...
\end{itemize}

\textbf{EWMA observado}: $\sim 6$--$8$ms (promedio)  
\textbf{Jitter observado}: $\sigma_{\text{latencia}} \sim 10$--$12$ms (desviación estándar)

El sistema parece \textit{nominalmente aceptable} según EWMA, pero el \textbf{jitter catastrófico} indica que el sistema está \textit{al borde del colapso}. En algunos ticks excede 20ms → backlog se acumula → predicciones obsoletas.

\textbf{Criterio de Nyquist para Jitter:}
...
Si el \textbf{jitter de latencia} (varianza) excede el 80\% del tick-rate, el sistema está en riesgo de perder sincronización:

\[
\text{Jitter}_{\text{threshold}} = 0.8 \times \text{besov\_nyquist\_interval\_ns}
\]

\textbf{Razón:} Si la desviación estándar de latencia es $\sim 80$ms y el tick-rate es 100ms, entonces:
\begin{itemize}
    \item $\mu_{\text{latencia}} - \sigma_{\text{latencia}} = 50 - 80 = -30$ms (algunos ticks son instantáneos)
    \item $\mu_{\text{latencia}} + \sigma_{\text{latencia}} = 50 + 80 = 130$ms (algunos ticks exceden el tiempo disponible)
\end{itemize}

Esto garantiza que al menos algunas predicciones serán \textit{obsoletas}, violando SLA de latencia.

\textbf{Solución: Telemetría de Alta Resolución con \texttt{perf\_counter\_ns}}

Python 3.7+ proporciona \texttt{time.perf\_counter\_ns()}, un reloj monotónico de alta resolución (nanosegundos) que no se ve afectado por ajustes de NTP o cambios de zona horaria:
...
class JitterMonitor:
    """
    Monitor de varianza de latencia (jitter) para detección precisa de saturación.
    Complementa el EWMA de latencia con métrica de dispersión.
    """
    def __init__(self, 
                 window_size: int = 100, 
                 nyquist_interval_ns: int = 100_000_000,  # 100ms
                 jitter_threshold_ratio: float = 0.8):
        """
        Args:
            window_size: Tamaño de ventana deslizante para calcular jitter
            nyquist_interval_ns: Intervalo mínimo entre ticks (Nyquist limit)
            jitter_threshold_ratio: Umbral de jitter como fracción del Nyquist (ej. 0.8 = 80%)
        """
        self.window_size = window_size
        self.nyquist_interval_ns = nyquist_interval_ns
        self.jitter_threshold_ns = jitter_threshold_ratio * nyquist_interval_ns
        
        # Historial de latencias (ventana deslizante)
        self.latency_history: Deque[int] = deque(maxlen=window_size)
        
        # Estadísticas acumuladas
        self.total_predictions = 0
        self.jitter_violations = 0
        self.max_latency_observed_ns = 0
        
    def record_latency(self, latency_ns: int) -> Dict:
        """
        Registra latencia de predicción y calcula jitter.
        
        Returns:
            Dict con métricas: jitter_ns, mean_ns, std_ns, violation, etc.
        """
        self.latency_history.append(latency_ns)
        self.total_predictions += 1
        self.max_latency_observed_ns = max(self.max_latency_observed_ns, latency_ns)
        
        # Calcular estadísticas solo si hay suficiente historia
        ...
        # Convertir a array numpy para cálculo eficiente
        latencies = jnp.array(list(self.latency_history), dtype=jnp.float64)
        
        mean_latency = jnp.mean(latencies)
        std_latency = jnp.std(latencies)  # Desviación estándar = jitter
        
        # Criterio de violación: jitter > 80% del tick-rate
        violation = std_latency > self.jitter_threshold_ns
        
        if violation:
            self.jitter_violations += 1
        
        # Recomendación de degradación (combina jitter + media)
        degradation_recommended = (
            violation or 
            (mean_latency > 0.7 * self.nyquist_interval_ns and std_latency > 0.5 * self.nyquist_interval_ns)
        )
        
        return {
            "jitter_ns": float(std_latency),
            "mean_ns": float(mean_latency),
            "std_ns": float(std_latency),
            "violation": bool(violation),
            "degradation_recommended": bool(degradation_recommended),
            "nyquist_limit_ns": self.nyquist_interval_ns,
            "jitter_threshold_ns": self.jitter_threshold_ns,
            "jitter_ratio": float(std_latency / self.nyquist_interval_ns) if self.nyquist_interval_ns > 0 else 0.0
        }
    
    def get_summary_stats(self) -> Dict:
        """Devuelve resumen estadístico completo."""
        ...
        return {
            ...
            "violation_rate": self.jitter_violations / self.total_predictions if self.total_predictions > 0 else 0.0,
            "mean_latency_ms": float(jnp.mean(latencies) / 1e6),
            "std_latency_ms": float(jnp.std(latencies) / 1e6),
            "min_latency_ms": float(jnp.min(latencies) / 1e6),
            "max_latency_ms": float(jnp.max(latencies) / 1e6),
            "p50_latency_ms": float(jnp.percentile(latencies, 50) / 1e6),
            "p95_latency_ms": float(jnp.percentile(latencies, 95) / 1e6),
            "p99_latency_ms": float(jnp.percentile(latencies, 99) / 1e6),
            "max_latency_ever_ms": float(self.max_latency_observed_ns / 1e6)
        }
\end{lstlisting}

\textbf{Integración con Load Shedding:}

El \texttt{JitterMonitor} complementa al \texttt{AdaptiveSignatureOrchestrator}, proporcionando señal de degradación más precisa:

\begin{lstlisting}[language=Python]
class AdaptiveSignatureOrchestrator_WithJitter:
    """
    Versión mejorada con telemetría de jitter.
    """
    def __init__(self, config: PredictorConfig):
        self.config = config
        self.tick_rate_ns = config.besov_nyquist_interval_ns
        
        # Orquestador original (EWMA de latencia)
        self.signature_graphs = {
            2: jax.jit(lambda path: compute_signature(path, depth=2)),
            3: jax.jit(lambda path: compute_signature(path, depth=3)),
            5: jax.jit(lambda path: compute_signature(path, depth=5)),
        }
        ...
        # NUEVO: Monitor de jitter
        self.jitter_monitor = JitterMonitor(
            window_size=100,
            nyquist_interval_ns=self.tick_rate_ns,
            jitter_threshold_ratio=0.8
        )
    ...
    def compute_adaptive_signature(self, path: jnp.ndarray) -> jnp.ndarray:
        """
        Calcula signature con profundidad adaptativa basada en EWMA + JITTER.
        """
        signature_fn = self.signature_graphs[self.current_depth]
        
        # Medir latencia con reloj de alta resolución
        t_start = time.perf_counter_ns()
        result = signature_fn(path)
        jax.block_until_ready(result)  # Forzar sincronización GPU
        t_end = time.perf_counter_ns()
        
        latency_ns = t_end - t_start
        
        # Actualizar EWMA (como antes)
        if self.ewma_latency_ns == 0.0:
            self.ewma_latency_ns = float(latency_ns)
        else:
            self.ewma_latency_ns = (
                self.ewma_alpha * latency_ns + 
                (1 - self.ewma_alpha) * self.ewma_latency_ns
            )
        
        # NUEVO: Actualizar monitor de jitter
        jitter_metrics = self.jitter_monitor.record_latency(latency_ns)
        
        # DECISIÓN DE DEGRADACIÓN (criterio combinado: EWMA + Jitter)
        latency_ratio = self.ewma_latency_ns / self.tick_rate_ns
        jitter_violation = jitter_metrics["violation"]
        jitter_ratio = jitter_metrics["jitter_ratio"]
        
        # Degradación agresiva si jitter es alto
        if jitter_violation and jitter_ratio > 1.0:
            # Jitter crítico: degradar directamente a M=2
            self.current_depth = 2
        elif latency_ratio >= 1.0 or jitter_metrics["degradation_recommended"]:
            # Latencia crítica O jitter recomendado: degradar a M=2
            self.current_depth = 2
        elif latency_ratio >= self.degradation_threshold or jitter_ratio > 0.6:
            # Advertencia: degradar a M=3
            self.current_depth = min(self.current_depth, 3)
        elif latency_ratio < self.recovery_threshold and jitter_ratio < 0.3:
            # Recuperación: ambos indicadores saludables
            self.current_depth = min(5, self.current_depth + 1)
        
        return result
    
    def get_telemetry(self) -> Dict:
        """Devuelve métricas combinadas: EWMA + Jitter."""
        jitter_summary = self.jitter_monitor.get_summary_stats()
        ...
        return {
            ...
            "tick_rate_ms": self.tick_rate_ns / 1e6,
            "latency_ratio": self.ewma_latency_ns / self.tick_rate_ns,
            "jitter_ms": jitter_summary.get("std_latency_ms", 0.0),
            "jitter_ratio": (jitter_summary.get("std_latency_ms", 0.0) * 1e6) / self.tick_rate_ns,
            "jitter_violations": jitter_summary.get("jitter_violations", 0),
            "violation_rate": jitter_summary.get("violation_rate", 0.0),
            "p99_latency_ms": jitter_summary.get("p99_latency_ms", 0.0),
            "status": (
                "DEGRADED_CRITICAL" if self.current_depth == 2 else
                "DEGRADED" if self.current_depth < 5 else
                "NOMINAL"
            )
        }
\end{lstlisting}

\textbf{Análisis de Métricas Combinadas:}

\begin{table}[h]
\centering
\begin{tabular}{|l|c|c|c|c|}
\hline
\textbf{Escenario} & \textbf{EWMA (ms)} & \textbf{Jitter (ms)} & \textbf{Decisión} & \textbf{$M$} \\
\hline
...
\textbf{Ventajas del Criterio Dual (EWMA + Jitter):}

\begin{enumerate}
    \item \textbf{Detección Temprana}: Jitter alto detecta problemas antes de que EWMA cruce umbral
    \item \textbf{Robustez a Outliers}: EWMA suaviza spikes, jitter los captura
    \item \textbf{Prevención de Flapping}: Criterio de recuperación requiere AMBOS indicadores saludables
    \item \textbf{Granularidad}: Jitter permite 3 niveles de degradación (5→3→2) en vez de binario
    \item \textbf{Visibilidad Operacional}: P99 latency expuesto para alertas de producción
\end{enumerate}

\textbf{Implementación de Medición Precisa:}

\begin{lstlisting}[language=Python]
class UniversalPredictor_WithJitterMonitoring:
    def __init__(self, config: PredictorConfig):
        self.config = config
        self.orchestrator = AdaptiveSignatureOrchestrator_WithJitter(config)
        # ...resto de inicialización...
    
    def predict_next(self, observation: MarketObservation) -> PredictionResult:
        """
        Predicción con telemetría de jitter de alta resolución.
        """
        # Tiempo de predicción TOTAL (incluye todos los kernels)
        t_prediction_start = time.perf_counter_ns()
        ...
        # 3. Compute signature con profundidad adaptativa (ya mide internamente)
        signature = self.orchestrator.compute_adaptive_signature(path)
        
        # 4. Resto de kernels (A, B, C)
        # ...
        
        # 5. Orquestador (fusión + JKO)
        predicted_value = self.orchestrator.predict_step(observation.price)
        
        # MEDIR LATENCIA TOTAL
        t_prediction_end = time.perf_counter_ns()
        total_latency_ns = t_prediction_end - t_prediction_start
        
        # Obtener telemetría
        telemetry = self.orchestrator.get_telemetry()
        
        # Logging condicional (solo si jitter es alto)
        if telemetry["jitter_ratio"] > 0.6:
            self.logger.warning(
                f"[JITTER ALERT] Ratio={telemetry['jitter_ratio']:.2f}, "
                f"P99={telemetry['p99_latency_ms']:.2f}ms, "
                f"Depth={telemetry['current_depth']}"
            )
        
        return PredictionResult(
            predicted_next=predicted_value,
            confidence=self._compute_confidence(signature),
            metadata={
                "signature_depth": telemetry["current_depth"],
                "ewma_latency_ms": telemetry["ewma_latency_ms"],
                "jitter_ms": telemetry["jitter_ms"],
                "jitter_ratio": telemetry["jitter_ratio"],
...
\textbf{Test de Jitter con Carga Variable:}

\begin{lstlisting}[language=Python]
def test_jitter_detection_under_variable_load():
    """
    Simula carga con latencia variable (jitter) para validar detección.
    """
    import random
    
    config = PredictorConfig(
        log_sig_depth=5,
        besov_nyquist_interval_ns=100_000_000  # 100ms
    )
    predictor = UniversalPredictor_WithJitterMonitoring(config)
    
    # Simular 200 predicciones con patrones de latencia diferentes
    latencies_observed = []
    depths_observed = []
    
    for i in range(200):
        # FASE 1 (i < 50): Latencia estable (0.8-1.2ms)
        if i < 50:
            simulated_processing_time_ns = random.uniform(800_000, 1_200_000)
        
        # FASE 2 (50 <= i < 100): Jitter alto (0.5-30ms)
        elif i < 100:
            simulated_processing_time_ns = random.uniform(500_000, 30_000_000)
        
        # FASE 3 (100 <= i < 150): Latencia alta constante (15-20ms)
        elif i < 150:
            simulated_processing_time_ns = random.uniform(15_000_000, 20_000_000)
        
        # FASE 4 (150 <= i < 200): Recuperación gradual (1-3ms)
        else:
            simulated_processing_time_ns = random.uniform(1_000_000, 3_000_000)
        ...
        # Inyectar latencia artificial
        time.sleep(simulated_processing_time_ns / 1e9)
        
        result = predictor.predict_next(observation)
        
        latencies_observed.append(result.metadata["total_latency_ns"] / 1e6)
        depths_observed.append(result.metadata["signature_depth"])
    
    # Assertions
    # FASE 1: Sistema debe mantener M=5 (jitter bajo)
    assert all(d == 5 for d in depths_observed[10:50]), "Debería mantener M=5 en fase estable"
    
    # FASE 2: Sistema debe degradar por jitter alto
    assert any(d < 5 for d in depths_observed[60:100]), "Debería degradar por jitter alto"
    
    # FASE 3: Sistema debe degradar por latencia alta
    assert all(d <= 3 for d in depths_observed[120:150]), "Debería degradar a M<=3 por latencia alta"
    
    # FASE 4: Sistema debe recuperar gradualmente
    final_depth = depths_observed[-10:]
    assert any(d >= 4 for d in final_depth), "Debería recuperar hacia M=5"
    
    # Estadísticas finales
    telemetry = predictor.orchestrator.get_telemetry()
    
    print("[✓] Jitter detection test PASSED")
    print(f"    Jitter violations: {telemetry['jitter_violations']}")
    print(f"    Violation rate: {telemetry['violation_rate']:.2%}")
    print(f"    P99 latency: {telemetry['p99_latency_ms']:.2f}ms")
    print(f"    Final depth: {telemetry['current_depth']}")
    print(f"    Final jitter ratio: {telemetry['jitter_ratio']:.2f}")
\end{lstlisting}

\textbf{Dashboard de Monitoreo (Prometheus + Grafana):}

\begin{lstlisting}[language=Python]
from prometheus_client import Histogram, Gauge, Counter

class UniversalPredictor_WithMetrics:
    def __init__(self, config: PredictorConfig):
        # ...inicialización...
        ...
        self.latency_histogram = Histogram(
            ...
            'Latency distribution of predictions',
            buckets=[0.001, 0.005, 0.010, 0.050, 0.100, 0.500, 1.0]
        )
        
        # Métricas de jitter
        self.jitter_gauge = Gauge(
            'predictor_jitter_milliseconds',
            'Standard deviation of latency (jitter)'
        )
        self.jitter_ratio_gauge = Gauge(
            'predictor_jitter_ratio',
            'Jitter as ratio of Nyquist limit'
        )
        self.jitter_violations_counter = Counter(
            'predictor_jitter_violations_total',
            'Total number of jitter threshold violations'
        )
        
        # Métricas de profundidad
        self.signature_depth_gauge = Gauge(
            'predictor_signature_depth',
            'Current signature computation depth (M)'
        )
    
    def predict_next(self, observation: MarketObservation) -> PredictionResult:
        with self.latency_histogram.time():
            result = self._predict_internal(observation)
        
        # Actualizar métricas de jitter
        telemetry = self.orchestrator.get_telemetry()
        self.jitter_gauge.set(telemetry["jitter_ms"])
        self.jitter_ratio_gauge.set(telemetry["jitter_ratio"])
        self.signature_depth_gauge.set(telemetry["current_depth"])
        
        if telemetry["jitter_ratio"] > 0.8:
            self.jitter_violations_counter.inc()
        
        return result
\end{lstlisting}

\textbf{Alertas Operacionales (Recomendadas):}

\begin{lstlisting}
# prometheus_alerts.yml
groups:
- name: predictor_latency
  interval: 30s
  rules:
  - alert: PredictorHighJitter
    expr: predictor_jitter_ratio > 0.8
    for: 1m
    labels:
      severity: warning
    annotations:
      summary: "High jitter detected in predictor"
      description: "Jitter ratio {{ $value }} exceeds 80% of Nyquist limit"
  
  - alert: PredictorJitterCritical
    expr: predictor_jitter_ratio > 1.2
    for: 30s
    labels:
      severity: critical
    annotations:
      summary: "Critical jitter in predictor"
      description: "Jitter ratio {{ $value }} exceeds Nyquist limit, predictions may be stale"
  
  - alert: PredictorDegraded
    expr: predictor_signature_depth < 5
    for: 5m
    labels:
      severity: info
    annotations:
      summary: "Predictor operating in degraded mode"
      description: "Signature depth reduced to {{ $value }} due to load"
\end{lstlisting}

\textbf{Conclusión:}

La \textbf{Telemetría de Jitter} es complemento esencial al Load Shedding basado en EWMA:

\begin{itemize}
    \item \textbf{EWMA solo}: Captura tendencia central, pero \textit{ignora} volatilidad
    \item \textbf{Jitter solo}: Detecta variabilidad, pero puede ser \textit{ruidoso} en régimen estable
    \item \textbf{EWMA + Jitter}: Combinación robusta que detecta \textit{tanto} sobrecarga sostenida como spikes intermitentes
\end{itemize}

El uso de \texttt{time.perf\_counter\_ns()} garantiza precisión de nanosegundos sin overhead significativo (<50ns por llamada). El criterio de umbral al 80\% del límite de Nyquist proporciona margen de seguridad antes de que el backlog se vuelva irrecuperable.

En producción, esta telemetría debe exponerse a Prometheus/Grafana para:
\begin{enumerate}
    \item Dashboards en tiempo real de salud del sistema
    \item Alertas automáticas cuando jitter excede umbrales
    \item Análisis post-mortem de incidentes de latencia
    \item Ajuste dinámico de \texttt{jitter\_threshold\_ratio} según carga de mercado
\end{enumerate}

Sin monitoreo de jitter, el sistema puede operar en \textit{modo degradado silencioso}, donde algunas predicciones son rápidas y otras lentas, promediando a un valor \textit{aparentemente} aceptable, pero violando SLA de latencia en percentiles altos (P95, P99).
