...
\noindent The system features dynamic module discovery without hardcoding, intelligent change detection for incremental testing (15$\times$ speedup in typical workflows), comprehensive JSON and Markdown reporting, and extensible architecture for adding new validation rules and tests.
\end{abstract}
...
\begin{enumerate}
    ...
    \item \textbf{Performance Optimization:} Detect only modified files since last run, reducing test time from 45s to $<$5s in typical workflows.
\end{enumerate}
...
\begin{enumerate}
    \item[0.] \textbf{Dependency Check:} \filehref{tests/scripts/dependency\_check.py} validates Golden Master compliance (always runs first)
    \item \textbf{Linting Phase:} \filehref{tests/scripts/code\_lint.py} validates style and formatting
    \item \textbf{Compliance Phase:} \filehref{tests/scripts/code\_alignement.py} audits 38 policies 
    \item \textbf{Execution Phase:} \filehref{tests/scripts/code\_structure.py} runs unit tests with real JAX execution
\end{enumerate}
...
jobs:
  test:
    ...
    steps:
      ...
      - run: ./tests/scripts/tests_start.sh --force --all
...
grep -A 5 '"passed": false' tests/results/code_structure_last.json

# Short summary
head -20 tests/reports/code_lint_last.md

# All compliance failures
grep "FAIL" tests/reports/code_alignement_last.md
\end{lstlisting}
...
The cache file \texttt{tests/.scope\_cache.json} stores modification timestamps of all Python files:
...
\hline
(default) & Process only modified files & Persistent \\
\texttt{--force} & Reset cache + process all files & Cleared, then regenerated \\
...
\begin{enumerate}
    ...
    \begin{itemize}[noitemsep]
        ...
        \item Verify \texttt{tests/results/} and \texttt{tests/reports/} directories
        \item Reset cache if \texttt{--force} flag detected
    \end{itemize}
    ...
    \begin{itemize}[noitemsep]
        ...
        \item List latest artifacts in \texttt{tests/results/}
...
\subsection{Orchestrator: \texttt{tests\_start.sh}}
...
\begin{itemize}[noitemsep]
    ...
    \item Parses CLI options (incremental vs. full audit)
...
\begin{itemize}[noitemsep]
    \item \textbf{JSON:} \texttt{tests/results/code\_alignement\_last.json}
    \begin{itemize}[noitemsep]
        \item Timestamp, policy count, pass/fail breakdown
        \item Per-policy result with details
    \end{itemize}
    
    \item \textbf{Markdown:} \texttt{tests/reports/code\_alignement\_last.md}
    \begin{itemize}[noitemsep]
...
\begin{itemize}[noitemsep]
    \item \textbf{Console:} pytest-formatted output with pass/fail summary
    \item \textbf{JSON:} \texttt{tests/results/code\_structure\_last.json} (structured results)
    \item \textbf{Markdown:} \texttt{tests/reports/code\_structure\_last.md} (detailed summary)
\end{itemize}
...
Cache file: \texttt{tests/.scope\_cache.json}
...
\begin{tabular}{|l|l|}
\hline
\textbf{Key} & Relative path from project root \\
\textbf{Value} & Unix timestamp (float with millisecond precision) \\
\hline
\end{tabular}
...
\hline
\textbf{Phase} & \textbf{Full} & \textbf{Incr (2 files)} & \textbf{Incr (0 files)} & \textbf{Unit} \\
\hline
Linting & 8.2s & 2.1s & 0.8s & sec \\
Compliance & 12.5s & 12.5s\textsuperscript{*} & 12.5s\textsuperscript{*} & sec \\
Execution & 24.3s & 24.3s\textsuperscript{*} & 24.3s\textsuperscript{*} & sec \\
\hline
\textbf{Total} & 45.0s & 39.0s\textsuperscript{†} & 37.8s\textsuperscript{†} & sec \\
\hline
\end{tabular}
\end{center}

\noindent\textsuperscript{*} = Policies and execution tests are repository/module-wide, not file-specific.

\noindent\textsuperscript{†} = Typical workflow improves startup time but not test phases.
...
\begin{enumerate}
    \item \textbf{Change Detection:} Cannot detect changes made by external tools (workaround: \texttt{--force})
