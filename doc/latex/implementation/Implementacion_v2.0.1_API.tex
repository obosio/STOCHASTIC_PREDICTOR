...
\begin{itemize}
    ...
    \item \textbf{Critical Fixes}: 
        \begin{itemize}
            ...
            \item [PENDING]: Zero-heuristics policy (expanded vector $\Lambda$)
        \end{itemize}
...
class PredictorConfig:
    """Complete Hyperparameter Vector Lambda (28 fields)."""
    # Metadata
    ...
    # Validation (Black Swan Detection)
    sigma_bound: float = 20.0  # N sigmas for outlier rejection
    
    # I/O Policies
    market_feed_timeout: int = 30
    market_feed_max_retries: int = 3
...
\textbf{Field Count}: 28 total fields (expanded from 15 in v2.0.1 initial release)
...
\textbf{Critical Refactor}: Removed hardcoded defaults from validation functions to enforce configuration-driven operation (Diamond Level Specification).

\begin{lstlisting}[language=Python]
# BEFORE (hardcoded heuristic):
def validate_price(
    price: Float[Array, "1"],
    sigma_bound: float = 20.0,  # MAGIC NUMBER
    sigma_val: float = 1.0
) -> Tuple[bool, str]:
    # ...

# AFTER (zero-heuristics):
def validate_price(
    price: Float[Array, "1"],
    sigma_bound: float,  # MUST come from PredictorConfig
    sigma_val: float = 1.0
) -> Tuple[bool, str]:
    """
    Zero-Heuristics Policy: sigma_bound MUST be passed 
    from PredictorConfig. No default value.
    """
    # ...
...
is_valid, msg = validate_price(
    ...
    sigma_bound=config.sigma_bound  # Explicit config injection
)
\end{lstlisting}

\textbf{Rationale}: Hardcoded \texttt{sigma\_bound = 20.0} violated the principle that ALL hyperparameters must reside in PredictorConfig. This prevented runtime tuning and broke the unity of the configuration vector $\Lambda$.
...
\textbf{Expanded from 15 to 28 fields} to enforce zero-heuristics policy.
...
\textbf{Expanded to 28 parameters} (13 new fields added for zero-heuristics compliance).

\begin{lstlisting}[language=bash]
# Core System Configuration
USP_CORE__STALENESS_TTL_NS=500000000

# Orchestration Parameters (13 total, 6 new)
USP_ORCHESTRATION__EPSILON=0.001
USP_ORCHESTRATION__LEARNING_RATE=0.01
USP_ORCHESTRATION__SINKHORN_EPSILON_MIN=0.01  # NEW
USP_ORCHESTRATION__SINKHORN_EPSILON_0=0.1     # NEW
USP_ORCHESTRATION__SINKHORN_ALPHA=0.5         # NEW
USP_ORCHESTRATION__ENTROPY_WINDOW=100         # NEW
USP_ORCHESTRATION__ENTROPY_THRESHOLD=0.8      # NEW
USP_ORCHESTRATION__SIGMA_BOUND=20.0           # NEW (outlier detection)
...
# Kernel Parameters (8 total, 4 new)
...
USP_IO__MARKET_FEED_TIMEOUT=30                # NEW
USP_IO__MARKET_FEED_MAX_RETRIES=3             # NEW
...
\textbf{Critical Fix (commits dc16b1a + 65e4bcf + [PENDING])}:
\begin{itemize}
    \item Replaced generic \texttt{JAX\_PLATFORMS} with \texttt{USP\_SECTION\_\_KEY} convention
    \item Documented ALL 28 algorithmic parameters with correct prefixes (expanded from 15)
...
\begin{itemize}
    ...
    \item ✓ \textbf{Config injection completeness}: All 28 PredictorConfig fields mapped (expanded from 15)
    ...
    \item ✓ \textbf{Zero-heuristics policy}: No hardcoded magic numbers (Diamond Level)
    \item ✓ \textbf{Validation API}: sigma\_bound no longer has default (must come from config)
\end{itemize}
...
\hline
Hardcoded heuristics (sigma\_bound) & [PENDING] & Removed from validate\_price() default \\
\hline
Missing config fields & [PENDING] & Expanded PredictorConfig to 28 fields \\
\hline
SDE/IO parameters ungoverned & [PENDING] & Added stiffness\_low/high, sde\_dt, market\_feed\_timeout, snapshot\_compression \\
\hline
\end{tabular}
\end{table}

\textbf{New Fields Added} (13 total):
\begin{itemize}
    ...
    \item \textbf{Kernels}: stiffness\_low, stiffness\_high, sde\_dt, sde\_numel\_integrations
    \item \textbf{I/O}: market\_feed\_timeout, market\_feed\_max\_retries, snapshot\_atomic\_fsync, snapshot\_compression (new section)
\end{itemize}
