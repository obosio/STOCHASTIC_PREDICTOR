...
\textbf{Field Count}: 31 total fields (expanded from 15 initial → 28 e4237ad → 31 current)
...
\begin{lstlisting}[language=Python]
# BEFORE (hardcoded heuristics - VIOLATIONS):
def validate_price(
    price: Float[Array, "1"],
...
# AFTER (zero-heuristics - COMPLIANT):
def validate_price(
    price: Float[Array, "1"],
...
\textbf{Rationale}: Hardcoded defaults for outlier detection (\texttt{sigma\_bound}, \texttt{sigma\_val}) and temporal drift validation (\texttt{max\_future\_drift\_ns}, \texttt{max\_past\_drift\_ns}) violated the Diamond Level principle that ALL hyperparameters must reside in PredictorConfig. This prevented runtime tuning for different market regimes (HFT vs. daily trading) and broke the unity of the configuration vector $\Lambda$.
