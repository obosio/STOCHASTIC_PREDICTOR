...
Per \texttt{Predictor\_Estocastico\_Python.tex §2.1}, the system enforces a strict 5-layer Clean Architecture:
...
│   └── __init__.py
│
└── tests/                  Layer 5: Validation (Hardware Parity)
    ├── conftest.py        
    ├── test_integration.py
    └── [subdirs for each layer]
\end{verbatim}
\end{figure}
...
\texttt{tests/} & Validation, fixtures, integration tests & Implementation logic \\
\hline
\end{tabular}
...
mkdir -p tests/{integrators,kernels,orchestrator,sia,utils}
touch tests/{__init__.py,conftest.py}
touch tests/{integrators,kernels,orchestrator,sia,utils}/__init__.py
\end{lstlisting}
...
\textbf{Spanish reserved for}:
\begin{itemize}
    \item LaTeX specification documents (\texttt{.tex}, \texttt{.pdf})
    \item Chat responses and external communication
\end{itemize}
...
Per \texttt{Predictor\_Estocastico\_Python.tex §1}:
...
│       ├── Implementacion_v2.0.2_Phase1_Complete.tex
│       └── [future phases]
│
└── pdf/
...
\begin{itemize}
    ...
    \item[✓] \texttt{tests/} directory with layer subdirectories
...
\begin{itemize}
    ...
    \item \texttt{tests/conftest.py}: 10 reusable pytest fixtures
\end{itemize}
