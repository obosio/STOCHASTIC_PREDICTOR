...
\begin{itemize}
    ...
    \item \textbf{Test Infrastructure} (\texttt{conftest.py}): 10 pytest fixtures for deterministic testing
\end{itemize}
...
\chapter{Test Infrastructure (conftest.py)}

\section{Overview}

The \texttt{conftest.py} module provides 10 pytest fixtures that establish:
\begin{itemize}
    \item Deterministic RNG seeding
    \item Synthetic data generation (Brownian motion, Lévy stable processes)
    \item Mock market data and reference solutions
    \item JAX device configuration
    \item Custom assertions
\end{itemize}

\section{PRNG Fixtures}
...
def rng_key():
    """Session-scoped RNG key for deterministic tests."""
    return initialize_jax_prng(seed=42)
\end{lstlisting}

\section{Data Generation Fixtures}

\subsection{synthetic\_brownian}

\begin{lstlisting}[language=Python]
@pytest.fixture
def synthetic_brownian(rng_key):
    """Generate synthetic Brownian motion path."""
    key1, key2 = split_key(rng_key)
    n_steps = 1000
    dt = 0.01
    increments = normal_samples(key2, n_steps)
    path = jax.numpy.cumsum(jax.numpy.sqrt(dt) * increments)
    return path
\end{lstlisting}

\subsection{synthetic\_levy\_stable}

\begin{lstlisting}[language=Python]
@pytest.fixture
def synthetic_levy_stable(rng_key):
    """Generate synthetic Lévy stable process."""
    # Generates alpha-stable increments
    # Uses Mantegna-Chambers algorithm
    # Returns normalized sample path
\end{lstlisting}

\section{Configuration Fixtures}

\subsection{default\_config}

\begin{lstlisting}[language=Python]
@pytest.fixture
def default_config():
    """Standard predictor configuration for tests."""
...
\subsection{test\_config\_low\_threshold}

\begin{lstlisting}[language=Python]
@pytest.fixture
def test_config_low_threshold():
    """Configuration with relaxed thresholds for quick testing."""
    # Same as default_config but with:
    # - warmup_steps=10
    # - n_particles=100
    # - cusum_threshold=2.0
\end{lstlisting}

\section{JAX Device Fixture}

\begin{lstlisting}[language=Python]
@pytest.fixture
def jax_device():
    """Ensure tests run on CPU for reproducibility."""
    jax.config.update('jax_platform_name', 'cpu')
    return jax.devices()[0]
\end{lstlisting}

\section{Assertion Fixture}

\subsection{assert\_jax\_deterministic}

\begin{lstlisting}[language=Python]
@pytest.fixture
def assert_jax_deterministic():
    """Custom assertion for deterministic output equality."""
    def _assert_equal(result1: Array, result2: Array, 
                      rtol: float = 1e-10):
        if not jax.numpy.allclose(result1, result2, rtol=rtol):
            raise AssertionError(
                f"Deterministic outputs differ:\n"
                f"  Max difference: {jax.numpy.max(jax.numpy.abs(result1 - result2))}"
            )
...
\begin{itemize}
    ...
    \item Comprehensive test infrastructure
\end{itemize}
