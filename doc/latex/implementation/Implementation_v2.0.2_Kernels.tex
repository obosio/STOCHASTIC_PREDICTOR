...
\begin{itemize}
    ...
    \item \textbf{Regime-Adaptive}: Different thresholds for crisis (σ > 0.2), normal (0.05 ≤ σ ≤ 0.2), and low-vol (σ < 0.05)
    \item \textbf{Zero-Heuristics}: All multipliers (γ_min, γ_max, γ_default) configurable in config.toml
...
\begin{enumerate}
    \item \textbf{V-CRIT-1}: Detects regime changes via κ_t spikes → triggers alarm with grace period
    \item \textbf{V-MAJ-1}: Adapts entropy threshold smoothly based on σ_t → detects mode collapse before regime change
    \item \textbf{Orchestrator}: Receives both signals (should_alarm from V-CRIT-1, gamma_t from V-MAJ-1) for comprehensive market intelligence
\end{enumerate}
