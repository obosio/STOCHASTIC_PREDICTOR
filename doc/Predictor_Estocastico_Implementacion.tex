...
El evento \texttt{RegimeChangedEvent} se dispara cuando la estadística de Page de la suma acumulada de residuos excede un umbral adaptativo.

\begin{algorithm}
\caption{Algoritmo CUSUM Discreto}
\begin{algorithmic}[1]
\State \textbf{Input:} Residuos estandarizados $e_t$, umbral $h$.
\State $S_0 \gets 0$, $G_0^+ \gets 0$, $G_0^- \gets 0$
\For{$t \gets 1$ \textbf{to} $N$}
    \State $G_t^+ \gets \max(0, G_{t-1}^+ + e_t - k)$
    \State $G_t^- \gets \max(0, G_{t-1}^- - e_t - k)$
    \If{$G_t^+ > h$ \textbf{or} $G_t^- > h$}
        \State \textbf{Emit} \texttt{RegimeChangedEvent}
        \State $G_t^+, G_t^- \gets 0$ (Reset CUSUM)
    \EndIf
\EndFor
\end{algorithmic}
\end{algorithm}
...
\begin{itemize}
    \item \textbf{Umbral CUSUM} ($h$): No debe ser una constante mágica. Debe definirse dinámicamente como $k \cdot \sigma_{\text{resid}}$, donde $\sigma_{\text{resid}}$ es la desviación estándar histórica de los residuos de predicción y $k \approx 3$ (regla de tres sigma).
