...
@jax.jit
def morlet_wavelet(t: Float[Array, ""], sigma: float, f_c: float) -> Float[Array, ""]:
    """
...
@jax.jit
def find_modulus_maxima(cwt_coeffs: Float[Array, "m n"], threshold: float = 0.1) -> Float[Array, "m n"]:
    """
...
def compute_partition_function(
    ...
    """
    _ = chain_magnitudes.shape[0]  # Number of positions/chains (not used, shape inferred)
...
def compute_singularity_spectrum(
    ...
    # Use concrete h_steps value (not traced) for jnp.linspace compatibility
    h_steps_concrete = 50  # Static value for JIT compilation
    h_range = jnp.linspace(h_min, h_max, h_steps_concrete)
...
def extract_holder_exponent_wtmm(signal: Float[Array, "n"], config) -> Float[Array, ""]:
    """
    ...
    holder_exponent_val = holder_exponent_raw * is_finite + config.wtmm_tau_default_scale * (1.0 - is_finite)
...
def compute_koopman_spectrum(
    signal: Float[Array, "n"], top_k: int, min_power: float, sampling_interval: float
...
def compute_paley_wiener_integral(
    signal: Float[Array, "n"], epsilon: float, sampling_interval: float
) -> Float[Array, ""]:
    """
...
def compute_malliavin_derivative(functional, signal: Float[Array, "n"]) -> Float[Array, "n"]:
    """
...
def compute_ocone_haussmann_representation(
    expected_value: Float[Array, ""], malliavin_derivative: Float[Array, "n"], dt: float
) -> Float[Array, ""]:
    """
...
@jax.jit
def gaussian_kernel(x: Float[Array, "d"], y: Float[Array, "d"], bandwidth: float) -> Float[Array, ""]:
    """
...
@jax.jit
def compute_gram_matrix(X: Float[Array, "n d"], bandwidth: float) -> Float[Array, "n n"]:
    """
...
def create_embedding(signal: Float[Array, "n"], config) -> Float[Array, "n_embed d"]:
    """
...
def kernel_a_predict(signal: Float[Array, "n"], key: Array, config) -> KernelOutput:
    ...
    y_pred_norm, variances = kernel_ridge_regression(X_train, y_train, X_test, config)
