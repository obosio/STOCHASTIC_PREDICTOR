...
def warmup_kernel_d_load_shedding(
    config: PredictorConfig, key: PRNGKeyArray, verbose: bool = False
) -> dict[int, float]:
    """
    ...
    for depth in emergency_depths:
        ...
        if verbose:
            mode_label = {2: "emergency", 3: "normal", 5: "rich"}.get(depth, "unknown")
...
def warmup_all_kernels(
    config: PredictorConfig, key: Optional[PRNGKeyArray] = None, verbose: bool = True
    ...
    if len(keys) < KernelType.N_KERNELS:
        raise ValueError(f"prng_split_count must be >= {KernelType.N_KERNELS}, got {len(keys)}")
...
def warmup_with_retry(
    config: PredictorConfig, max_retries: int = 3, verbose: bool = True
) -> dict[str, float | dict[int, float]]:
    """
...
def profile_warmup_and_recommend_timeout(
    config: PredictorConfig, verbose: bool = True
