...
class ProcessStateSchema(BaseModel):
    ...
    state_tag: Optional[str] = Field(
        default=None,
        description="Process state label (e.g., 'high_variance', 'stationary', 'trending')",
    )
    dispersion_proxy: Optional[Float[Array, "1"]] = Field(
        default=None, description="Realized dispersion estimate for Sinkhorn coupling"
    )
    ...
    def validate_bounds(cls, value, info):
        ...
        if "reference_prediction" in info.data:
            ...
            if jnp.any(value > ref_pred):
                raise ValueError("confidence_lower must be <= reference_prediction")
        return value
