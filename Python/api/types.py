...
class PredictorConfig:
    ...
    def __post_init__(self):
        ...
        assert 0.0 < self.learning_rate <= 1.0, f"learning_rate must be in (0, 1], got {self.learning_rate}"
        assert self.jko_domain_length > 0.0, f"jko_domain_length must be > 0, got {self.jko_domain_length}"
        assert self.entropy_window_relaxation_factor > 0.0, "entropy_window_relaxation_factor must be > 0"
        assert self.entropy_window_bounds_min > 0, "entropy_window_bounds_min must be > 0"
        assert (
            self.entropy_window_bounds_max >= self.entropy_window_bounds_min
        ), "entropy_window_bounds_max must be >= entropy_window_bounds_min"
        assert 0.0 < self.learning_rate_safety_factor <= 1.0, "learning_rate_safety_factor must be in (0, 1]"
        assert self.learning_rate_minimum > 0.0, "learning_rate_minimum must be > 0"

        # Entropic regularization must be positive
        assert self.epsilon > 0, f"epsilon must be > 0 (Sinkhorn), got {self.epsilon}"
        assert self.sinkhorn_epsilon_min > 0, f"sinkhorn_epsilon_min must be > 0, got {self.sinkhorn_epsilon_min}"
        assert (
            self.sinkhorn_epsilon_0 >= self.sinkhorn_epsilon_min
        ), f"sinkhorn_epsilon_0 must be >= epsilon_min, got {self.sinkhorn_epsilon_0}"
        assert 0.0 < self.sinkhorn_alpha <= 1.0, f"sinkhorn_alpha must be in (0, 1], got {self.sinkhorn_alpha}"
        assert self.sinkhorn_inner_iterations > 0, "sinkhorn_inner_iterations must be > 0"
        assert self.sinkhorn_cost_type in (
            "squared",
            "huber",
        ), "sinkhorn_cost_type must be 'squared' or 'huber'"
        assert self.sinkhorn_huber_delta > 0.0, "sinkhorn_huber_delta must be > 0"

        # Entropy monitoring constraints
        assert self.entropy_window > 0, f"entropy_window must be > 0, got {self.entropy_window}"
        assert 0.0 < self.entropy_threshold <= 1.0, f"entropy_threshold must be in (0, 1], got {self.entropy_threshold}"
        assert (
            0.0 <= self.entropy_volatility_low_threshold < self.entropy_volatility_high_threshold
        ), "entropy_volatility thresholds must satisfy 0 <= low < high"
        assert (
            0.0 < self.entropy_ratio_min <= self.entropy_ratio_max
        ), "entropy_ratio bounds must satisfy 0 < min <= max"
        assert self.entropy_baseline_floor > 0.0, "entropy_baseline_floor must be > 0"
        assert self.entropy_scaling_trigger > 0.0, "entropy_scaling_trigger must be > 0"
        assert self.koopman_top_k > 0, "koopman_top_k must be > 0"
        assert self.koopman_min_power > 0.0, "koopman_min_power must be > 0"
        assert self.paley_wiener_integral_max > 0.0, "paley_wiener_integral_max must be > 0"
        assert self.wtmm_num_scales > 0, "wtmm_num_scales must be > 0"
        assert self.wtmm_scale_min > 0.0, "wtmm_scale_min must be > 0"
        assert self.wtmm_sigma > 0.0, "wtmm_sigma must be > 0"
        assert self.wtmm_fc > 0.0, "wtmm_fc must be > 0"
        assert self.wtmm_modulus_threshold > 0.0, "wtmm_modulus_threshold must be > 0"
        assert self.wtmm_max_link_distance > 0.0, "wtmm_max_link_distance must be > 0"
        assert self.wtmm_q_steps > 1, "wtmm_q_steps must be > 1"
        assert self.wtmm_q_max > self.wtmm_q_min, "wtmm_q_max must be > wtmm_q_min"
        assert self.wtmm_h_steps > 1, "wtmm_h_steps must be > 1"
        assert self.wtmm_h_max > self.wtmm_h_min, "wtmm_h_max must be > wtmm_h_min"
        assert self.wtmm_tau_default_scale > 0.0, "wtmm_tau_default_scale must be > 0"
        assert self.kernel_a_min_wiener_hopf_order > 0, "kernel_a_min_wiener_hopf_order must be > 0"

        # Log-signature depth reasonable (exponential complexity)
        assert 3 <= self.log_sig_depth <= 5, f"log_sig_depth must be in [3, 5], got {self.log_sig_depth}"

        # SDE integration parameters
        assert self.sde_dt > 0, f"sde_dt must be > 0, got {self.sde_dt}"
        assert self.sde_numel_integrations > 0, f"sde_numel_integrations must be > 0, got {self.sde_numel_integrations}"
        ...
        assert dt_upper_bound > 0.0, "CFL validation: sde_pid_dtmax must allow safe timesteps"

        assert (
            self.stiffness_low > 0 and self.stiffness_high > self.stiffness_low
        ), f"stiffness thresholds must satisfy 0 < low < high, got {self.stiffness_low}, {self.stiffness_high}"
        assert self.sde_fd_epsilon > 0.0, "sde_fd_epsilon must be > 0"
        assert self.kernel_c_jump_intensity >= 0.0, "kernel_c_jump_intensity must be >= 0"
        assert self.kernel_c_jump_scale >= 0.0, "kernel_c_jump_scale must be >= 0"
        assert self.kernel_c_jump_max_events > 0, "kernel_c_jump_max_events must be > 0"

        # Holder exponent bounds (stochastic processes)
        assert 0.0 < self.holder_threshold < 1.0, f"holder_threshold must be in (0, 1), got {self.holder_threshold}"
        assert self.robustness_dimension_threshold > 0.0, "robustness_dimension_threshold must be > 0"
        assert 0.0 < self.dgm_entropy_coupling_beta <= 1.0, "dgm_entropy_coupling_beta must be in (0, 1]"
        assert self.dgm_max_capacity_factor >= 1.0, "dgm_max_capacity_factor must be >= 1"
        assert self.stiffness_calibration_c1 > 0.0, "stiffness_calibration_c1 must be > 0"
        assert self.stiffness_calibration_c2 > 0.0, "stiffness_calibration_c2 must be > 0"
        assert self.stiffness_min_low > 0.0, "stiffness_min_low must be > 0"
        assert self.stiffness_min_high > self.stiffness_min_low, "stiffness_min_high must be > stiffness_min_low"
        assert 0.0 < self.holder_exponent_guard < 1.0, "holder_exponent_guard must be in (0, 1)"

        assert self.telemetry_buffer_capacity > 0, "telemetry_buffer_capacity must be > 0"
        assert self.telemetry_hash_algorithm in (
            "sha256",
            "crc32c",
        ), "telemetry_hash_algorithm must be 'sha256' or 'crc32c'"
        assert self.telemetry_adaptive_log_path, "telemetry_adaptive_log_path must be non-empty"
        assert self.signal_sampling_interval > 0.0, "signal_sampling_interval must be > 0"
        assert self.degraded_recovery_min_steps > 0, "degraded_recovery_min_steps must be > 0"
        assert self.telemetry_adaptive_window_size > 0, "telemetry_adaptive_window_size must be > 0"
        assert self.telemetry_dashboard_width > 0, "telemetry_dashboard_width must be > 0"
        assert self.telemetry_dashboard_height > 0, "telemetry_dashboard_height must be > 0"
        assert self.telemetry_dashboard_spread_epsilon > 0.0, "telemetry_dashboard_spread_epsilon must be > 0"
        assert self.telemetry_dashboard_recent_rows > 0, "telemetry_dashboard_recent_rows must be > 0"
        assert self.frozen_signal_variance_floor > 0.0, "frozen_signal_variance_floor must be > 0"

        # CUSUM thresholds
        assert self.cusum_h > 0 and self.cusum_k >= 0, "CUSUM thresholds must be non-negative"

        # Outlier detection & temporal drift
        assert self.sigma_bound > 0, f"sigma_bound must be > 0, got {self.sigma_bound}"
        assert self.sigma_val > 0, f"sigma_val must be > 0, got {self.sigma_val}"
        assert self.max_future_drift_ns > 0, f"max_future_drift_ns must be > 0, got {self.max_future_drift_ns}"
        assert self.max_past_drift_ns > 0, f"max_past_drift_ns must be > 0, got {self.max_past_drift_ns}"

        # I/O constraints
        assert self.data_feed_timeout > 0, f"data_feed_timeout must be > 0, got {self.data_feed_timeout}"
        assert self.data_feed_max_retries >= 0, f"data_feed_max_retries must be >= 0, got {self.data_feed_max_retries}"
        assert self.snapshot_compression in {
            "none",
            "gzip",
            "brotli",
        }, f"snapshot_compression must be 'none', 'gzip', or 'brotli', got '{self.snapshot_compression}'"
        assert self.snapshot_format in {
            "msgpack",
            "protobuf",
        }, f"snapshot_format must be 'msgpack' or 'protobuf', got '{self.snapshot_format}'"
        assert self.snapshot_hash_algorithm in {
            "sha256",
            "crc32c",
        }, f"snapshot_hash_algorithm must be 'sha256' or 'crc32c', got '{self.snapshot_hash_algorithm}'"
        assert (
            self.telemetry_hash_interval_steps > 0
        ), f"telemetry_hash_interval_steps must be > 0, got {self.telemetry_hash_interval_steps}"
        assert (
            self.frozen_signal_min_steps >= 5
        ), f"frozen_signal_min_steps must be >= 5, got {self.frozen_signal_min_steps}"
        assert (
            self.frozen_signal_recovery_ratio > 0
        ), f"frozen_signal_recovery_ratio must be > 0, got {self.frozen_signal_recovery_ratio}"
        assert (
            self.frozen_signal_recovery_steps > 0
        ), f"frozen_signal_recovery_steps must be > 0, got {self.frozen_signal_recovery_steps}"

        # Stability parameter validation bounds
        assert self.validation_alpha_stable_min >= 0.0, "validation_alpha_stable_min must be >= 0"
        assert self.validation_alpha_stable_max <= 2.0, "validation_alpha_stable_max must be <= 2"
        assert self.validation_beta_stable_min >= -1.0, "validation_beta_stable_min must be >= -1"
        assert self.validation_beta_stable_max <= 1.0, "validation_beta_stable_max must be <= 1"
        assert self.validation_viscosity_residual_max > 0.0, "validation_viscosity_residual_max must be > 0"

        # TTL/Nyquist coherence
        assert self.staleness_ttl_ns > 0 and self.besov_nyquist_interval_ns > 0, "Latency timeouts must be positive"

        # PRNG defaults
        assert self.prng_seed >= 0, "prng_seed must be >= 0"
        assert self.prng_split_count >= 4, "prng_split_count must be >= number of kernels"
...
class PredictionResult:
    ...
    def __post_init__(self):
        ...
        assert lower <= ref <= upper, "confidence_lower must be <= reference_prediction <= confidence_upper"
