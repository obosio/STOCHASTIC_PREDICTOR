...
def validate_timestamp(timestamp_ns: int, max_future_drift_ns: int, max_past_drift_ns: int) -> Tuple[bool, str]:
    """
...
def check_staleness(timestamp_ns: int, config: "PredictorConfig") -> Tuple[bool, int]:
    """
...
def validate_shape(array: jnp.ndarray, expected_shape: Tuple[int, ...], name: str) -> Tuple[bool, str]:
    """
    ...
    if len(actual_shape) != len(expected_shape):
        return False, (
            f"{name}: Expected {len(expected_shape)} dimensions, " f"got {len(actual_shape)} (shape: {actual_shape})"
        )
    ...
    for i, (actual, expected) in enumerate(zip(actual_shape, expected_shape)):
        if expected != -1 and actual != expected:
            return False, (
                f"{name}: Dimension {i} mismatch - expected {expected}, " f"got {actual} (full shape: {actual_shape})"
            )

    return True, "Valid"


def validate_finite(array: jnp.ndarray, name: str, allow_nan: bool, allow_inf: bool) -> Tuple[bool, str]:
    """
...
def validate_simplex(weights: Float[Array, "N"], atol: float, name: str) -> Tuple[bool, str]:
    """
    ...
    if not jnp.isclose(total, 1.0, atol=atol):
        return False, (f"{name}: Does not sum to 1.0 (sum={total:.6f}, " f"error={abs(total - 1.0):.2e})")
...
def validate_holder_exponent(H: Union[float, Float[Array, "1"]], min_val: float, max_val: float) -> Tuple[bool, str]:
    """
    ...
    if H_val < min_val or H_val > max_val:
        return False, (f"Holder exponent out of range: {H_val:.4f} not in [{min_val}, {max_val}]")

    return True, "Valid"


def validate_alpha_stable(alpha: float, min_val: float, max_val: float, exclusive_bounds: bool) -> Tuple[bool, str]:
    """
    ...
    Args:
        ...
        exclusive_bounds: If True, use (min, max] instead of [min, max]
            - from config.validation_alpha_stable_exclusive_bounds
    ...
    if exclusive_bounds:
        if alpha <= min_val or alpha > max_val:
            return False, (f"Alpha parameter out of range: {alpha:.4f} not in ({min_val}, {max_val}]")
    else:
        if alpha < min_val or alpha > max_val:
            return False, (f"Alpha parameter out of range: {alpha:.4f} not in [{min_val}, {max_val}]")
...
def validate_beta_stable(beta: float, min_val: float, max_val: float) -> Tuple[bool, str]:
    """
    ...
    if beta < min_val or beta > max_val:
        return False, (f"Beta parameter out of range: {beta:.4f} not in [{min_val}, {max_val}]")
...
def warn_if_invalid(is_valid: bool, message: str, exception_type: Union[type, None]) -> None:
    """
...
def cast_array_to_float64(array: Float[Array, "..."], warn_if_downcast: bool = True) -> Float[Array, "..."]:
    """
