...
def load_meta_optimization_config() -> MetaOptimizationConfig:
    ...
    if missing:
        raise ValueError("Missing required [meta_optimization] entries in config.toml: " f"{sorted(missing)}")
...
def _coerce_float(value: Any, label: str) -> float:
    ...
    if isinstance(value, (int, float, np.floating)):
        return float(value)
    raise ConfigMutationError(f"Non-numeric value for '{label}' in config mutation: {value}")
...
class BayesianMetaOptimizer:
    ...
    def __init__(
        ...
        if not OPTUNA_AVAILABLE:
            raise ImportError("Optuna is required for meta-optimization. " "Install with: pip install optuna>=3.0")
    ...
    def optimize(
        ...
        if self.base_config is not None and self.base_config.prng_seed is not None:
            ...
            if "prng_seed" not in core_section:
                raise ValueError(
                    "Missing required config: [core].prng_seed. " "Zero-Heuristics policy forbids silent defaults."
                )
    ...
    def get_best_config(self) -> Optional[PredictorConfig]:
        ...
        return replace(self.base_config, **self.study.best_params)
...
def walk_forward_split(
    ...
    if fold_size < min_fold_size:
        ...
        warnings.warn(
            ...
            f"Using smaller folds. Reduce n_folds or increase data_length. "
    ...
    if stratify_by_volatility:
        if data is None:
            raise ValueError("stratify_by_volatility=True requires data argument")
    ...
    for i in range(n_folds):
        ...
        if stratify_by_volatility and volatility is not None and volatility_tertiles is not None:
            ...
            mid_vol_count = np.sum(
                (val_volatility >= volatility_tertiles[0]) & (val_volatility <= volatility_tertiles[1])
            )
            ...
            if low_pct < min_representation or mid_pct < min_representation or high_pct < min_representation:
...
def compute_rolling_volatility(data: np.ndarray, window: int = 20) -> np.ndarray:
    """
