...
def parse_requirement_line(
    line: str, platform_info: Dict[str, str]
...
def evaluate_marker(marker: str, platform_info: Dict[str, str]) -> bool:
    ...
    if "sys_platform" in marker:
        marker_eval = marker_eval.replace(
            "sys_platform", f"'{platform_info['sys_platform']}'"
        )

    # Replace platform_machine
    if "platform_machine" in marker:
        marker_eval = marker_eval.replace(
            "platform_machine", f"'{platform_info['platform_machine']}'"
        )
...
def load_requirements(
    root: Path, platform_info: Dict[str, str], req_file: str = "requirements.txt"
) -> Dict[str, str]:
    ...
    with open(requirements_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line_stripped.startswith("-r"):
                ...
                included_reqs = load_requirements(
                    root, platform_info, str(included_path)
                )
    ...
    try:
        ...
        for package_name, expected_version in sorted(requirements.items()):
            ...
            if status == "PASS":
                ...
                print(
                    f"✗ {package_name:20s} expected: {expected_version}, installed: {installed_version} [MISMATCH]"
                )
...
def save_markdown_report(results: Dict, output_path: Path) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"**Platform:** {results['platform']['sys_platform']} ({results['platform']['platform_machine']})\n\n"
        )

        # Executive Summary
        f.write("## 📊 Executive Summary\n\n")
        status_icon = (
            "✅" if (results["failed"] == 0 and results["missing"] == 0) else "❌"
        )
        status_text = (
            "PASS" if (results["failed"] == 0 and results["missing"] == 0) else "FAIL"
        )
        success_rate = (
            (results["passed"] / results["total_packages"] * 100)
        ...
        for pkg in results["packages"]:
            ...
            f.write(
                f"| {pkg['package']} | {pkg['expected']} | {pkg['installed']} | {status_icon} {pkg['status']} |\n"
            )
        ...
        if results["failed"] == 0 and results["missing"] == 0:
            ...
            f.write(
                f"❌ **{results['failed'] + results['missing']} package(s) do not match requirements.txt**\n\n"
            )
