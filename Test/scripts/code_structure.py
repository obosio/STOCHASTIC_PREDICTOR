...
from Python.core.sinkhorn import SinkhornResult, compute_sinkhorn_epsilon
...
class TestAPIPRNG:
    ...
    def test_uniform_samples(self, prng_key):
        ...
        samples = uniform_samples(prng_key, shape=(10,), minval=0.0, maxval=1.0, dtype=jnp.float64)
        assert samples.shape == (10,)
        assert samples.dtype == jnp.float64

    def test_normal_samples(self, prng_key):
        """Execute: normal_samples()."""
        samples = normal_samples(prng_key, shape=(10,), mean=0.0, std=1.0, dtype=jnp.float64)
        assert samples.shape == (10,)
        assert samples.dtype == jnp.float64

    def test_exponential_samples(self, prng_key):
        """Execute: exponential_samples()."""
        samples = exponential_samples(prng_key, shape=(10,), rate=1.0, dtype=jnp.float64)
        assert samples.shape == (10,)
        assert samples.dtype == jnp.float64
...
class TestAPITypes:
    ...
    def test_ProcessState(self):
        ...
        ps = ProcessState(magnitude=jnp.array([1.0]), timestamp_utc=datetime.now(timezone.utc))
...
class TestAPIValidation:
    ...
    def test_validate_magnitude(self):
        ...
        is_valid, msg = validate_magnitude(magnitude=0.5, sigma_bound=10.0, sigma_val=1.0, allow_nan=False)
...
class TestAPISchemas:
    ...
    def test_ProcessStateSchema(self):
        ...
        schema = ProcessStateSchema(magnitude=jnp.array([0.5]), timestamp_utc=datetime.now(timezone.utc))
...
class TestCoreOrchestrator:
    ...
    def test_compute_adaptive_stiffness_thresholds(self, config_obj):
        ...
        lower, upper = compute_adaptive_stiffness_thresholds(holder_exponent=jnp.array(0.5), config=config_obj)
        assert lower is not None
        assert upper is not None

    def test_compute_adaptive_jko_params(self, config_obj):
        """Execute: compute_adaptive_jko_params()."""
        n_iter, epsilon = compute_adaptive_jko_params(volatility_sigma_squared=0.5, config=config_obj)
...
class TestCoreFusion:
    ...
    def test_compute_sinkhorn_epsilon(self, config_obj):
        ...
        epsilon = compute_sinkhorn_epsilon(ema_variance=jnp.array([0.5]), config=config_obj)
    ...
    def test_walk_forward_split(self):
        ...
        for i, (train_indices, test_indices) in enumerate(splits):
            ...
            if i > 0:
                ...
                assert (
                    curr_train_size > prev_train_size
...
def main() -> int:
    """Execute pytest and generate JSON report in Test/results/."""
    import io
    import json
    import re
    import sys
    ...
    # Parse CLI arguments
    parser = argparse.ArgumentParser(description="Structural execution tests with cache-aware reporting")
    parser.add_argument(
        "--force-all",
        action="store_true",
    ...
    def truncate_output(text: str, max_chars: int = 20000) -> str:
        """Trim pytest output to keep JSON size reasonable."""
    ...
    total = passed + failed
    # Note: success_rate could be calculated but not currently used in payload
    # success_rate = (passed / total * 100) if total > 0 else 0

    # Trim output for JSON payload size control
    trimmed_output = truncate_output(output_text)

    # Create JSON report
    python_files = sorted(
        {
            str(path.relative_to(project_root))
            for path in (project_root / "Python").rglob("*.py")
            if "__pycache__" not in path.parts
        }
    )
    python_modules = sorted(
        {
            str(path.relative_to(project_root)).replace("/", ".").replace(".py", "")
            for path in (project_root / "Python").rglob("*.py")
            if "__pycache__" not in path.parts and path.name != "__init__.py"
        }
    )
    files = [str(Path(__file__).relative_to(project_root))] + python_files
    ...
    json_report = {
        "metadata": {
            "report_id": "code_structure",
            "timestamp_utc": timestamp,
            "status": "PASS" if exit_code == 0 else "FAIL",
            "source": "Test/scripts/code_structure.py",
            "framework_version": "2.1.0",
            "notes": "Structural execution tests for Python modules",
        },
        "summary": {
            ...
            "metrics": [
                ...
                {"label": "DurationSeconds", "value": f"{duration:.2f}"},
            ],
        },
        "scope": {
            "targets": {
                "folders": ["Python", "Test/scripts"],
                "files": files,
