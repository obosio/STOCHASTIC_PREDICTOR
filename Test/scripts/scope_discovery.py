...
def get_cache_file() -> Path:
    ...
    Located in: Test/.scope_cache.json
    """
    root = get_root()
    return root / "Test" / ".scope_cache.json"
...
def discover_changed_files(
    root: Path | str | None = None, force_all: bool = False
) -> List[str]:
...
def discover_module_files_changed(
    module_name: str, root: Path | str | None = None, force_all: bool = False
) -> List[str]:
    ...
    module_changed = [Path(f).name for f in all_changed if module_path in f]
...
def discover_module_files(
    module_name: str, root: Path | str | None = None
) -> List[str]:
